...
def check_update():
    try:
        ...
        response = subprocess.check_output(["pip", "search", "--no-cache-dir", "--json", "cerbogen"]).decode("utf-8")
        data = json.loads(response)
        latest_version = data["info"][0]["version"]
        
        # Compare versions
        if installed_version == latest_version:
            messagebox.showinfo("Check for Updates", "No updates available")
        else:
