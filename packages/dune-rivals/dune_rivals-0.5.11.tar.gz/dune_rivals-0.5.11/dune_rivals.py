...
class BaseSardaukar(BaseRivalActor):
    ...
    def start(self, spice_loc_map: np.ndarray, spice_file_map: np.ndarray, order_map: dict):
        ...
        num_warriors_fields = {idx: 0 for idx in [1, 2, 3, 4]}
        spice = np.where(self.spice_file_map==OBJ_FILE)
        for i, j in zip(spice[0], spice[1]):
            warriors = self.order_map[(i, j)]
            num_warriors_fields[len(warriors)] += 1
        print(f"NUM_WARRIORS_FIELDS: {num_warriors_fields}")
        print("---")
        print(f"TOTAL FIELDS <=3: {num_warriors_fields[1] + num_warriors_fields[2] + num_warriors_fields[3]}")
        print("---")
        print(f"TOTAL FIELDS: {num_warriors_fields[4]}")
