...
class Database(local, NodeClassRegistry):
    ...
    def _object_resolution(self, result_list, result_class):
        """
            ...
            for a_result_attribute in enumerate(a_result_item[1]):                    
                try:
                    ...
                    if type(a_result_attribute[1]) is list:
                        resolved_object = self._object_resolution([a_result_attribute[1]], result_class)                    
                    
                    result_list[a_result_item[0]][a_result_attribute[0]] = resolved_object
                    
                except KeyError:
                    r_labels = a_result_attribute[1].labels
                    label_match = False
                    for key,value in self._NODE_CLASS_REGISTRY.items():
                        if value == result_class:
                            label_match = all([k_label in r_labels for k_label in key])
                    if not label_match:
                    ...
                    resolved_object = result_class.inflate(a_result_attribute[1])
                    result_list[a_result_item[0]][a_result_attribute[0]] = resolved_object
    ...
    @ensure_connection
    def cypher_query(self, query, params=None, handle_unique=True, retry_on_session_expire=False, resolve_objects=False, retry_failed_connection=0, result_class=None):
        """
        ...
        try:
            ...
            if resolve_objects:
                ...
                results = self._object_resolution(results, result_class)
