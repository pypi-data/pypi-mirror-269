...
class QueryBuilder(object):
    ...
    def _execute(self, lazy=False):
        ...
        query = self.build_query()
        results, _ = db.cypher_query(query, self._query_params, resolve_objects=True, result_class=self._ast['result_class'])            
...
class NodeSet(BaseSet):
    ...
    def _get(self, limit=None, lazy=False, **kwargs):
        ...
        if limit:
            self.limit = limit
        obj = self.query_cls(self)
        return obj.build_ast()._execute(lazy)
