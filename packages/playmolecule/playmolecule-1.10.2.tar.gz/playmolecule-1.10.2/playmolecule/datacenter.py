...
def download_dataset(
    ...
    while not downloaded and attempts > 0:
        ...
        try:
            result = session._request_url_token(
                "get", f"/datacenter/{datasetid}", headers, _logger=_logger
            )
...
def get_datasets(
    ...
    r = session._request_url_token("get", "/datacenter", headers=headers, _logger=False)
    if r is None:
        return
...
def remove_dataset(session, datasetid, _logger=True):
    ...
    r = session._request_url_token(
        "delete", f"/datacenter/{datasetid}", _logger=_logger
    )
...
def move_dataset(session, datasetid, remotepath, _logger=True):
    ...
    r = session._request_url_token(
        "put", f"/datacenter/{datasetid}", headers=headers, _logger=_logger
    )
...
class DataCenter:
    ...
    def upload_dataset(
        ...
        with open(tmpfile, "rb") as fh:
            ...
            while not uploaded and attempts > 0:
                try:
                    result = self._session._request_url_token(
                        "post", "/datacenter", files=files, data=data, _logger=_logger
                    )
                except Exception as e:
                    attempts -= 1
                    _throw_error(
    ...
    def get_dataset_tags(self, datasetid, _logger=True):
        ...
        r = self._session._request_url_token(
            "get", f"/datacenter/{datasetid}/tags", _logger=_logger
        )
    ...
    def remove_dataset_tag(self, datasetid, tag, _logger=True):
        ...
        r = self._session._request_url_token(
            "delete", f"/datacenter/{datasetid}/tags/{tag.lower()}", _logger=_logger
        )
