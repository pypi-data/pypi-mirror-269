...
def extract_dependencies(element):
    return element.dependencies(recurse=False)
...
def make_cc_library(element, manifest, includes=None, **extra):
    ...
    deps = [
        dep.normal_name + "_cc" for dep in element.dependencies(recurse=False)
    ]
    data = [element.normal_name]
    ...
    for item in manifest:
        _, ext = posixpath.splitext(item)
        if SRC_EXT.fullmatch(ext, 1):
            # looks like a source
            srcs.add(item)
