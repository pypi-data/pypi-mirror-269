...
class MessageLocal(Item, CompoundMessage, metaclass=MetaLogger, object=object_message):
    ...
    def __init__(self, *, recipients: List[Recipient], api_type_id: int = None, campaign_id: int = None,
                 ...
                 is_http_api: bool = None, endpoint: str = None, debug: bool = False,
        ...
        CompoundMessage.__init__(self, message_id=message_id, campaign_id=campaign_id, debug=debug,
    ...
    # TODO get_body_json_after_text_template()  [what should it include?]
    def get_body_after_text_template(self, recipient: Recipient,
        ...
        if message_channel is None:
            ...
            self.logger.info(object={"message_channel": message_channel})

        body = ""  # TODO: is this the right way to do it?
        for block in self.get_profile_blocks(recipient.get_profile_id(), message_channel):
            preferred_language = block.get("preferred_language")
            if block["question_per_language"][preferred_language]:
                body += block["question_per_language"][preferred_language] + "\n"
            elif block["body_per_language"][preferred_language]:
                body += block["body_per_language"][preferred_language] + "\n"
        return body
