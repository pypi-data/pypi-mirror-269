...
class Input(InputBase):
    ...
    def start_and_end_date(
        ...
        value: str | None = start_date_value
        format: str | None = None if e(value) else get_date_format(nns(value))
        start_date: datetime | None = PIH.DATA.EXTRACT.datetime(value, nns(format))
        if ne(start_date):
            if format == CONST.YEARLESS_DATE_FORMAT:
                start_date = nnt(start_date).replace(today.year)
        value = end_date_value
        format = None if e(value) else get_date_format(value)
        end_date: datetime | None = PIH.DATA.EXTRACT.datetime(value, nns(format))
        if ne(end_date):
            if format == CONST.YEARLESS_DATE_FORMAT:
        ...
        while True:
            if e(start_date):
                ...
                start_date = PIH.DATA.EXTRACT.datetime(value, CONST.DATE_FORMAT)
            ...
            if e(end_date) or start_date > end_date:
                if not self.yes_no(
                    ...
                    if format == CONST.YEARLESS_DATE_FORMAT:
                        value = jp((value, today.year))
                    end_date = PIH.DATA.EXTRACT.datetime(value, CONST.DATE_FORMAT)
                    if e(end_date):
                        continue
                else:
    ...
    class RESULT(ResultTool):
        ...
        class EVENTS:
            ...
            def get(
                ...
                parameters: tuple[Any, ...] | None = None,
                timestamp: datetime | date | str | int | list[date | datetime] | None = None,
                count_as_result: bool = False,
            ) -> Result[list[EventDS] | int]:
