...
class DynamodbStorageClient(AbstractStorageClient):
    ...
    def get_by_pages(
        ...
        yield from batched(self.iterate_table_items(
            page_size=page_size,
    ...
    def iterate_table_item_models(self, item_model_type: BaseDynamodbModel, table_items: list[dict]):
        for annotated_table_item in self.iterate_annotated_responses(table_items=table_items):
    ...
    def iterate_table_items(
        ...
        for page in paginator.paginate(**scan_params, ConsistentRead=consistent_read):
            ...
            if item_model_type:
                yield from self.iterate_stored_item_models(table_items=table_items, item_model_type=item_model_type)
            else:
