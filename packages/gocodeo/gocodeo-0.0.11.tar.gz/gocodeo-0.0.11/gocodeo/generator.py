...
def generate_api_request(code, context, type, number,import_lines,imported_code,src_path,test_path):
    ...
    prompt = f"""You are angularjs unit testing expert. Your job is to write {number} numbers high quality unit test code using jasmine and karma testing framework only, for given angular code: {code} for the given {type} scenarios in the context: {context}.
      ...
      covering all scenarios in one file. Please mock and spy all the services and data using jasmine and karma testing framework only where ever necessary. 
