...
GoCodeo_URL = "https://dev-ai-service.gocodeo.com/api/v1/explain_api"
...
def update_client_1(private_key, total_lines, total_functions):
    ...
    # if private_key == "gc_spgrXSnfZqYSVqqhQiVjlYApSQofFXIUt1":
...
def explain_api_request(code):
    ...
    prompt =f"You are reactjs unit testing expert. Your job is to first understand the given code {code} then explain the code so that every team member should understand the logic and working of the function"
    payload = {
...
def generate_api_request(code, context, type, number,import_lines,imported_code,src_path,test_path):
    ...
    prompt = f"""You are reactjs unit testing expert. Your job is to write {number} numbers high quality unit test code using react testing library and jest testing framework only, for given react code: {code} for the given {type} scenarios in the context: {context}.
      ...
      covering all scenarios in one file. Please mock and spy all the services and data using react testing library and jest testing framework only where ever necessary. 
      ...
      in the source code file present at {src_path} location. Don't forgot to import the source code file for which test cases are generated.
...
def generate_api_request1(code, response, type, number):
    prompt = f"You are reactjs unit testing expert.Here is the current generated unit test cases:{response}, for input code:{code} . Your task is to improve code of all these current unit test cases on {type} type scenarios. If the unit test cases you are getting that are  partially implemented ,then you have to fully implement it accurately using react testing library and jest testing framework only. Also check if all the imports that are used are there at the top.It is mandatory that every unit test cases should be implemented completely."
    # print(prompt)
    try:
        prompt1 = prompt
        parameters = {
                ...
                if file_size >= 30000:
                    ...
                    for obj in behaviour_type:
                        ...
                        if test_response1:
                            ...
                            pattern1 = r'(?:\s*```(?:typescript|javascript|ts|jsx|tsx)?\n)?(.*?)(?:```|$)'
                            pattern2 = r'```(?:typescript|javascript|ts|jsx|tsx)?\n(.*?)```|\b(?:typescript|javascript|ts|jsx|tsx)?\n(.*?)(?=$|```)'
                            combined_pattern = f"{pattern1}|{pattern2}"
                            ...
                            if match:
                                ...
                                content=test_response1
                            new_imports=extract_user_defined_imports(content)
                            # print(new_imports,"in new imports")
                            updated_code= ''
                            if new_imports:
                                    updated_code=replace_string_imports(content, new_imports)
                            else:
                                    updated_code=replace_string_imports(content, import_lines)
