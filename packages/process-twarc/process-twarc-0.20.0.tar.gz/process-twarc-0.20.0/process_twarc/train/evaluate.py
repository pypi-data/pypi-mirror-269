...
def check_results(
    ...
    def check_trials(
        ...
        for group, df in trials.items():
            ...
            def evaluate(run_name, parameters, datasets, tokenizer):
                ...
                if not trainer:
                    return None
                all_metrics = parameters["metrics"]

                results = {}
                for split in ["development", "test"]:
                    values = trainer.evaluate(datasets[split])
                    for metric in all_metrics:
                        results[f"{split}_{metric}"] = values[f"eval_{metric}"]
                        print(f"{split}_{metric}: {values[f'eval_{metric}']}")
    ...
    def generate_summary_sheet(trials):
        ...
        for group, df in checked.items():
            df = df[[col for col in df.columns if any(x in col for x in ["epoch", "development", "test", "value"])]]
