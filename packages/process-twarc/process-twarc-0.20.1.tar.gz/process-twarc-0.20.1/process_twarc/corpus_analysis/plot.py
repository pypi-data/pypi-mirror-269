...
def  plot_tSNE(
        ...
        colors: list=plt.cm.get_cmap('tab10')(np.linspace(0, 1, 10)),
    ...
    def preprocess_data(users_agg=users_agg, colors=colors):
        ...
        for column in [("absolute", "sum"), ("absolute", "cum_sum"), ("relative", "sum"), ("relative", "cum_sum")]:
            if column[0] == "absolute":
                if max(df[column]) > 1e6:
                    df[column] = df[column].apply(lambda x: '{:.1f}M'.format(x/1e6))
                elif max(df[column]) > 1e3:
                    df[column] = df[column].apply(lambda x: str(x) if x < 1000 else '{:.0f}k'.format(x/1000))
            else:
...
def plot_stacked_histogram_of_corpus_by_user_contributions(
    ...
    def plot(data_dict):
        ...
        fig, ax = plt.subplots(figsize=(10, n * 2))  # Adjust the height based on the number of bars
