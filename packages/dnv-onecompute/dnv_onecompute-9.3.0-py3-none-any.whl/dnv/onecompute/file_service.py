...
class ProgressInfo(NamedTuple):
    ...
    Attributes:
        source (str): The path of the source file being processed.
        destination (str): The path where the file is being copied to.
        current (int): The current progress, in bytes, of the operation.
        total (int): The total size, in bytes, of the file being processed.
    """

    class Status(Enum):
        """An enumeration representing the status of a file operation."""

        QUEUE = 0  # The operation is queued and waiting to be processed.
        IGNORED = 1  # The operation is ignored and not processed.
        IN_PROGRESS = 2  # The operation is in progress.
...
class FileTransferOptions:
    ...
    Attributes:
        patterns (list of str)
            A list of file patterns to search for. Each pattern can contain wildcard characters
            such as '*' and '?'. If multiple patterns are provided, files that match any of the
            patterns will be returned.
        min_size (Optional[str])
            An optional size threshold in human-readable format. Files that are smaller than this
            threshold will be excluded from the results. Default value is '0B'.
        max_size (Optional[str])
            An optional size threshold in human-readable format. Files that are bigger than this
            threshold will be excluded from the results. Default value is set with no upper limits.
    """

    patterns: list[str] = field(default_factory=lambda: ["**/*.*"])
    min_size: Optional[str] = None  # If None the default is 0KB
    max_size: Optional[str] = None  # If None the default is not limit
...
class FileService(ABC):
    ...
    Attributes:
        ProgressCallback (Callable[[ProgressInfo], None]):
            ...
            real-time feedback on the progress of an operation. It takes a single argument of type
            ProgressInfo.
