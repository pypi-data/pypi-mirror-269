...
class LocalWorkflowRuntimeServiceManager:
    ...
    Args:
        workspace_id (str): The identifier for the workspace where workflows will be executed.
        worker_host_apps_path (str): The path to the worker host applications path.
        workflow_runtime_service_endpoint (str, optional): The endpoint URL for the workflow
            runtime service. If not explicitly specified, the default value is set to
            'http://localhost:{port}', where '{port}' represents an available and unoccupied
            port number. This default configuration ensures that the workflow runtime service
            is accessible locally on an automatically assigned port.
        workflow_runtime_executable_path (str): The path to the workflow runtime executable.
        workflow_runtime_log_level (LogLevel, optional): The log level for the workflow runtime
            service. Default is LogLevel.NONE.
        workflow_runtime_log_filename (str, optional): The name of the log file. If provided, logs
            will be saved to this file. If not specified (set to 'None'), a default file named
            "WC.LOG" will be created in the working directory of the LocalWorkflowRuntime Service
            started with. This option can also be an absolute path to the file, in which case it
            will be saved at the desired location.
        workflow_debugging (bool, optional): Whether to enable debugging for workflows.
            Default is False.
        auto_deploy_option (AutoDeployOption, optional): The auto-deployment option for workflows.
            Default is AutoDeployOption.RELEASE.
        max_concurrent_workers (int, optional): The maximum number of concurrent workers.
            Default is None.
        blob_storage_path (str, optional): The optional path where blob data is stored.
            If not specified, the default path on Windows is under the '%TEMP%' directory
            ('$userprofile\\AppData\\Local\\Temp'). The folder is named with a prefix of 'oc_'
            followed by a unique identifier and suffixed with '_blob'.
            The specified blob storage path must be a valid and writable directory on the
            filesystem, accessible by the user running the workflow runtime.
        jobs_root_path (str, optional): The optional path where the root directory for jobs is
            set up. If not specified, the default path on Windows is located within the '%TEMP%'
            directory ('$userprofile\\AppData\\Local\\Temp'). This default folder is named with a
            prefix of 'oc_', followed by a unique identifier, and suffixed with '_jobs'.
            The provided 'jobs_root_path' must be a valid and writable directory on the filesystem,
            accessible by the user running the workflow runtime.
        temp_folder_path (str, optional): The path for storing temporary files.
        ...
        startup_wait_time (int, optional): The startup wait time in seconds. Default is 5 seconds.
        console_window_visible (bool): A boolean flag indicating whether the LocalWorkflowRuntime
            service should run with or without a visible console window. Defaults to False,
            indicating that the console window is not visible.
        redirect_console_logs_to_terminal (bool): Flag indicating whether to redirect console logs
            to the terminal. Default to True.
        clean_temporary_directories_on_exit (bool): Flag indicating whether to clean up the local
            temporary directories on exit. Default to True.
    """
    ...
    # The working directory formed by combining 'jobs_root_path' and 'workspace-id'. This directory
    # is used for managing job-specific files and resources within the context of a workspace. It is
    # created by combining the 'jobs_root_path' with the unique 'workspace-id'.
