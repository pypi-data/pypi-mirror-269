...
from typing import Generator, Optional, Set, Dict, Tuple, List, Any
...
from jf_ingest.config import IssueMetadata, IssueListForDownload
...
def fetch_id_to_key_for_updated(
    ...
    for proj_id, issues in project_id_to_issue.items():
        if len(issues) == 0:
            ...
            last_run_date_with_buffer = (last_run_date - timedelta(days=2)).strftime(
                '%Y-%m-%d'
            )
        ...
        issue_count = _get_all_project_issue_counts(
            jira_connection,
            [str(proj_id)],
            datetime.strptime(last_run_date_with_buffer,
                              "%Y-%m-%d"),
            10)
        jql_expression = f"project = {proj_id} AND updatedDate > '{last_run_date_with_buffer}' " f"ORDER BY id ASC"
...
def fetch_id_to_key_for_all_existing(
        jira_connection: JIRA, project_id_to_issues: Dict[int, set[IssueMetadata]], earliest_issue_dt: datetime,
) -> Dict[int, str]:
...
def get_issue_list_to_download(
    ...
    * deletes since last pull:
    ...
    logger.info(
        f"{len(id_to_key_updated)} "
        f"remote issues updated across all projects since last pull"
    )
    ...
    logger.info(f"{len(id_to_key_on_local)} issues local, {len(id_to_key_on_remote)} issues remote, "
                f"{len(id_to_key_deleted)} issues deleted")

    # all changed key
    id_to_key_changed = {}
    for issue_id in id_to_key_on_local:
        if issue_id in id_to_key_on_remote and id_to_key_on_local[issue_id] != id_to_key_on_remote[issue_id]:
    ...
    if full_redownload:
        logger.info("`full_redownload` is true, ALL local issues will be queued for download from remote")
