...
def reconcile(*paths: str, overwrite: bool = False) -> None:
    ...
    if len(paths) > 0:
        prompt = f"Agree to {done.info.warn('remove')} conflicts?" + 2 * os.linesep
        if overwrite or done.info.dilemma(prompt + done.info.indent(os.linesep.join(paths))):
            remove(*paths)
        else: exit()
