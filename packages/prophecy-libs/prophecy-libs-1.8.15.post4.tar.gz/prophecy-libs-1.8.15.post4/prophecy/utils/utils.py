...
def instrument():
    def decorator(function):
        def inner_wrapper(spark, *args, **kwargs):
            ...
            sendGemProgressEvent(spark, user_session, process_id, state.value.__str__(), start_time)

            try:
                state = TaskState.RUNNING
                sendGemProgressEvent(spark, user_session, process_id, state.value.__str__(),
                                     start_time)
