...
def sendGemProgressEvent(spark: SparkSession, userSession: str, gemName: str, state: str,
    ...
    if exception:
        spark.sparkContext._jvm.org.apache.spark.sql.ProphecySparkSession.sendGemProgressEvent(
            spark._jsparkSession, userSession, gemName, state, startTime, endTime, exception
        )
    else:
        spark.sparkContext._jvm.org.apache.spark.sql.ProphecySparkSession.sendGemProgressEvent(
...
def withInstrumentation():
    def decorator(function):
        def inner_wrapper(spark, *args, **kwargs):
            ...
            sendGemProgressEvent(spark, user_session, gem_name, state.value.__str__(), start_time)

            try:
                state = TaskState.RUNNING
                sendGemProgressEvent(spark, user_session, gem_name, state.value.__str__(),
                                     start_time)
