...
class AstronomicalSeeing:
    ...
    async def calculate_seeing_model4(self, temperature, humidity, wind_speed, altitude):
        ...
        # seeing_factor = CONSTANT_C * (water_vapor_pressure / 10) ** 0.5 * (wind_speed / 10) ** 0.5 * relative_pressure
        # print((water_vapor_pressure / 10) ** 0.25, (wind_speed / 10) ** 0.75, relative_pressure, (water_vapor_pressure / 10) ** 0.25 * (wind_speed / 10) ** 0.75 * relative_pressure)
