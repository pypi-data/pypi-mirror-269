...
class AstroWeather:
    ...
    async def _get_location_data(self) -> None:
        ...
        for row in self._weather_data_seventimer:
            ...
            seeing = 0
            if row["seeing"] == -1:
                seeing = await self._astro_seeing.calculate_seeing_model4(
                    temperature=details.get("air_temperature", -1),
                    humidity=details.get("relative_humidity", -1),
                    wind_speed=details.get("wind_speed", -1),
                    altitude=self._elevation,
                )
            else:
                seeing = SEEING[row["seeing"] - 1]
            ...
            item = {
                ...
                # Met.no
                "cloudcover": details.get("cloud_area_fraction", -1),
                ...
                "condition_percentage": await self.calc_condition_percentage(
                    ...
                    seeing,
                    # await self._astro_seeing.calculate_seeing_model4(
                    #     temperature=details.get("air_temperature", -1),
                    #     humidity=details.get("relative_humidity", -1),
                    #     wind_speed=details.get("wind_speed", -1),
                    #     altitude=self._elevation,
    ...
    async def _get_forecast_data(self, forecast_type, hours_to_show) -> None:
        ...
        await self.retrieve_data_seventimer()
        await self.retrieve_data_metno()
        now = datetime.now(timezone.utc).replace(tzinfo=None)    #utcnow()
        ...
        for row in self._weather_data_seventimer:
            ...
            if (
                ...
                for i in range(0, 3):
                    ...
                    item["cloudcover"] = details.get("cloud_area_fraction", -1)
