...
class AstroWeather:
    ...
    async def _get_location_data(self) -> None:
        ...
        for row in self._weather_data_seventimer:
            ...
            item = {
                ...
                # 7timer
                "seeing": await self._astro_seeing.calculate_seeing_model4(temperature=details.get("air_temperature", -1), humidity=details.get("relative_humidity", -1), wind_speed=details.get("wind_speed", -1), altitude=self._elevation),
                ...
                "condition_percentage": await self.calc_condition_percentage_new(
                    details.get("cloud_area_fraction", -1),
                    await self._astro_seeing.calculate_seeing_model4(temperature=details.get("air_temperature", -1), humidity=details.get("relative_humidity", -1), wind_speed=details.get("wind_speed", -1), altitude=self._elevation), #row["seeing"],
                    row["transparency"],
        ...
        for row in self._weather_data_seventimer:
            ...
            if (
                ...
                for i in range(0, 3):
                    ...
                    item["seeing"] = await self._astro_seeing.calculate_seeing_model4(temperature=details.get("air_temperature", -1), humidity=details.get("relative_humidity", -1), wind_speed=details.get("wind_speed", -1), altitude=self._elevation)
                    item["condition_percentage"] = await self.calc_condition_percentage_new(
                        item["cloud_area_fraction"],
                        await self._astro_seeing.calculate_seeing_model4(temperature=details.get("air_temperature", -1), humidity=details.get("relative_humidity", -1), wind_speed=details.get("wind_speed", -1), altitude=self._elevation),
                        row["transparency"],
                        item["wind_speed"],
                    )
    ...
    async def _get_deepsky_forecast(self):
        ...
        for day in range(0, len(start_indexes)):
            ...
            for idx in range(
                ...
                cloud_area_fraction = row.cloud_area_fraction_percentage
                ...
                _LOGGER.debug(
                    ...
                    str(transparency),
                    str(wind_speed),
                    await self.calc_condition_percentage_new(cloud_area_fraction, seeing, transparency, wind_speed),
                )
    ...
    async def calc_condition_percentage_new(self, cloudcover, seeing, transparency, wind_speed):
        """Return condition based on cloud cover, seeing, transparency, and wind speed"""
        # Possible Values:
        #   Clouds: 1-9
        #   Seeing: 1-8
        ...
        seeing = int(100 - seeing * 40)  # Seeing is something in between 0 and 2.5 arcsecs
