...
class AstronomicalSeeing:
    ...
    async def calculate_seeing_model0(self, temperature, wind_speed, humidity, altitude):
        ...
        _LOGGER.debug("AstronomicalSeeing calculation model4: T %s, W %s, H %s, E %s - S %s", str(temperature), str(wind_speed), str(humidity), str(altitude), str(seeing))
    ...
    def calculate_dew_point(self, temperature, humidity):
        a = 17.62  # MAGNUS_COEFFICIENT_A
        b = 243.12  # MAGNUS_COEFFICIENT_B
        ...
        # alpha = float(Decimal(str(humidity / 100)).ln()) + MAGNUS_COEFFICIENT_A * temperature / (MAGNUS_COEFFICIENT_B + temperature)
        # dew_point = (MAGNUS_COEFFICIENT_B * alpha) / (MAGNUS_COEFFICIENT_A - alpha)

        # alpha = humidity / 100. * math.exp(a * temperature / (b + temperature))
        # print(alpha)
        # dew_point = b * math.log(alpha) / (a - math.log(alpha))
    ...
    def calculate_saturation_vapor_pressure(self, dew_point):
        ...
        print("++++ saturation_vapor_pressure: " + str(es))
        return es
    ...
    def calculate_water_vapor_pressure(self, temperature, humidity):
        ...
        print("++++ water_vapor_pressure: " + str(es))
    ...
    def calculate_water_vapor_pressure_b(self, temperature, humidity, dew_point):
        # dew_point = self.calculate_dew_point(temperature, humidity)
        es = self.calculate_saturation_vapor_pressure(dew_point)
        water_vapor_pressure = (humidity / 100) * es
        return water_vapor_pressure

    async def calculate_seeing_model4b(self, temperature, humidity, wind_speed, dew_point, altitude):
        water_vapor_pressure = self.calculate_water_vapor_pressure_b(temperature, humidity, dew_point)
        adjusted_pressure = 1013.25 * math.exp(-0.00012 * altitude)
        relative_pressure = adjusted_pressure / 1013.25
        ...
        if seeing > 2.5:
            seeing = 2.5
        print("----" + str(dew_point))
        _LOGGER.debug("AstronomicalSeeing calculation model4b: T %s, W %s, H %s, D %s,  E %s - S %s", str(temperature), str(wind_speed), str(humidity), str(dew_point), str(altitude), str(seeing))
        return seeing
