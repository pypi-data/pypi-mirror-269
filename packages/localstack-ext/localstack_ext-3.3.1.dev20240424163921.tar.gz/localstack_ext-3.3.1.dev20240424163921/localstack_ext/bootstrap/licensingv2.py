...
class LicensedPluginLoaderGuard(PluginLifecycleListener):
	...
	def on_init_after(C,plugin_spec,plugin):
		...
		if not C.environment.has_product_license(B):
			if A.namespace!=PLATFORM_PLUGIN_NAMESPACE:LOG.warning(f"Disabled plugin {B} since it is not part of the current license agreement 🔑❌")
			raise PluginDisabled(A.namespace,A.name)
