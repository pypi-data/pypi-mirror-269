...
Borrowed from https://github.com/sorend/sshconf
"""
import re
...
class SSHConfigFile(object):
    ...
    def host(self, host):
        ...
        if host in self.hosts_:
            ...
            for k, value in [
                (x.key.lower(), x.value) for x in self.lines_ if x.host == host and x.key.lower() != "host"
            ]:
                vals[k].append(value)
    ...
    def set(self, host, **kwargs):
        ...
        **kwargs : The new configuration parameters
        ...
        for key, values in kwargs.items():
            ...
            lower_key = key.lower()
            update_idx = [idx for idx, x in enumerate(self.lines_) if x.host == host and x.key.lower() == lower_key]
            extra_remove = []
            ...
            if values:
                ...
                for value in values:
                    self.lines_.insert(
                        max_idx + 1,
                        ConfigLine(line=self._new_line(mapped_key, value), host=host, key=mapped_key, value=value),
                    )
    ...
    def unset(self, host, *args):
        ...
        *args : list of settings to removes.
        ...
        remove_idx = [idx for idx, x in enumerate(self.lines_) if x.host == host and x.key.lower() in args]
...
def load_ssh_config_file(path) -> SSHConfigFile:
    """
...
def test_ssh_connection(hostname: str):
