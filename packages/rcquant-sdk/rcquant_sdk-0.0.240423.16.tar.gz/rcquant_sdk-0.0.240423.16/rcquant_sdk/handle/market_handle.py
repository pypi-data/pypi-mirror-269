...
class MarketHandle():
    def __init__(self, tsocket: TSocket):
        ...
        self.__Tick_Columns_Types = {"ExchangeID": str, "InstrumentID": str, "TradingDay": str, "ActionDay": str,
                                     "ActionTime": str, "ActionMSec": np.int64, "LastPrice": np.float64,
                                     "LastVolume": np.int64, "BidPrice": np.float64, "BidVolume": np.int64,
                                     "AskPrice": np.float64, "AskVolume": np.int64, "TotalTurnover": np.float64,
                                     "TotalVolume": np.int64, "OpenInterest": np.int64, "PreClosePrice": np.float64,
                                     "PreSettlementPrice": np.float64, "PreOpenInterest": np.int64}
        self.__OHLC_Columns_Types = {
            "ExchangeID": str, "InstrumentID": str, "TradingDay": str, "ActionDay": str, "ActionTime": str,
            "Period": np.int64, "OpenPrice": np.float64, "HighestPrice": np.float64, "LowestPrice": np.float64,
            "ClosePrice": np.float64, "CloseVolume": np.int64, "CloseBidPrice": np.float64,
            "CloseAskPrice": np.float64, "CloseBidVolume": np.int64, "CloseAskVolume": np.int64,
            "TotalTurnover": np.float64, "TotalVolume": np.int64, "OpenInterest": np.int64,
            "PreClosePrice": np.float64, "PreSettlementPrice": np.float64, "PreOpenInterest": np.int64}
    ...
    def fin_save_ohlc_list(self, instrument_id: str, period: str, df, **kwargs) -> Tuple[bool, str]:
        ...
        b, m, params = self.__create_fin_persists_save_param_data(instrument_id, period, df, kwargs=kwargs)
        if b == False:
            return [b, m]
    ...
    def fin_save_tick_list(self, instrument_id, period, df, **kwargs) -> Tuple[bool, str]:
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]
        ...
        b, m, params = self.__create_fin_persists_save_param_data(instrument_id, period, df, **kwargs)
        if b == False:
            return [b, m]
    ...
    def __create_fin_persists_save_param_data(self, instrument_id: str, period: str, df, **kwargs):
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]
