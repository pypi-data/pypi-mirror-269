...
def fin_save_ohlc_list(instrument_id: str, period: str, df, **kwargs):
    """批量按天保存OHLC数据
    ...
    return FinClient.instance().market_handle().fin_save_ohlc_list(instrument_id, period, df, **kwargs)


def fin_read_ohlc_list(instrument_id: str, period: str, start_date: int, end_date: int, base_path: str = '', is_return_list: bool = False):
    '''
    获取ohlc数据
    :param instrument_id:合约编码
    :param period:周期
    ...
    return FinClient.instance().market_handle().fin_read_ohlc_list(FinPersistReadParamData(
        instrument_id=instrument_id, period=period, start_date=start_date, end_date=end_date, base_path=base_path), is_return_list=is_return_list)


def fin_save_tick_list(instrument_id, period, df, **kwargs):
    """批量按天保存行情数据
    ...
    Returns:
        _type_: [bool,str]
    """
    return FinClient.instance().market_handle().fin_save_tick_list(instrument_id, period, df, **kwargs)


def fin_read_tick_list(instrument_id: str, period: str, start_date: int = 0, end_date: int = 99999999, base_path: str = '', is_return_list: bool = False):
    '''
    获取basetick数据
    :param instrument_id:合约编码
    :param period:周期
