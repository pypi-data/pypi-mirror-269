...
import traceback
import collections
from typing import Callable, Hashable, TYPE_CHECKING, TypeVar
...
class _PortRegistryMeta(type):
    ...
    def __call__(cls: type[__singleton_instance_type], *args, **kwargs) -> __singleton_instance_type:
        """
        Args:
        ...
        try:
            ...
            instance = super(_PortRegistryMeta, cls).__call__(*args, **kwargs)
...
class Input(Port):
    ...
    def _send_input_msg_to_calls(self, msg: 'Msg') -> None:
        ...
        if self.is_enabled:
            ...
            midiscripter.base.shared.thread_executor.map(self.__call_worker,
