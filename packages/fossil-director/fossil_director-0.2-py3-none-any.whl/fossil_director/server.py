"""
Multi-threaded SCGI/HTTP server for dispatching fossil repositories 
...
class FossilProxyHandler(StreamRequestHandler):
    ...
    def handle(self):
        ...
        if command:
            response = self.run_fossil(command, buf)
        else:
            response = b"400 Bad Request\r\n\r\n"
        self.wfile.write(response)
        self.wfile.close()

    def run_fossil(self, command: str, buf: bytes) -> bytes:
...
class FossilProxyServer(ThreadingTCPServer):
    ...
    def get_repo(self, hostname: str) -> list[str]:
