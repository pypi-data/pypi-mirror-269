...
class JobIntf:
    ...
    async def download_eir(self, session_data: SessionData, order: EirOrder):
        ...
        try:
            async with ClientSession() as cs:
                async with cs.get(url, headers=headers, data=data.model_dump_json(), verify_ssl=False) as resp:
                    data = await resp.content.read()
                    buffer = io.BytesIO(data)
                    headers = {'Content-Disposition': f'attachment; filename="{order.bill_no}.pdf"'}  # 下载文件
