...
class RespType(Enum):
    ...
    task_delay = -2  # 任务延迟执行，比如下一次执行时间5min后,加入redis控制
    task_stock_limit = -3  # 无库存
    session_invalid_token = -11  # 无效token
    session_invalid_user_pw = -12  # 无效用户名
    session_request_limit = -21  # 访问次数受限
    network_timeout_error = -22  # 代理服务器错误,或超时
...
class ResponseData(BaseModel):
    ...
    log: Optional[bool] = True  # 是否打印日志
    action: Optional[ActionType] = None  # 动作类型
    session_data: Optional[SessionData] = None

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        log = f'{self.msg} {self.action.name if self.action else ""} {self.data if self.data else ""}{"/n" + str(self.session_data) if self.session_data else ""}{"/n" + self.response if self.response else ""}'
...
class CtnInfo(BaseModel):
    ...
    apply_data: Optional[Any] = None
    barcode: Optional[str] = None
    print_account: Optional[str] = None
    print_data: Optional[Any] = None
...
class EirOrder(BaseModel):
    ...
    is_canceled: Optional[Any] = False
    is_disabled: Optional[Any] = False
    is_completed: Optional[Any] = False
    ...
    def to_order_for_print(self, account) -> EirOrderForPrint:
        ctn_ids = list(map(lambda x: x.ctn_id, self.ctns))
