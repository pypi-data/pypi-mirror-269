    ...
    def __init__(self, **kwargs):
        ...
        if self.code == RespType.task_failed and (self.msg or self.data or self.response):
            ...
            if self.code == RespType.task_success and self.action in [ActionType.print_eir, ActionType.apply_eir]:
                ...
                logger.warning(self.log_text())
            else:
                logger.info(self.log_text())

    def log_text(self):
        result = [f'{self.msg}  {self.code.name if self.code else ""} {self.action.name if self.action else ""}']
        if self.session_data:
            result.append(self.session_data)
        if self.response:
            result.append(self.response)
        if self.data:
            if issubclass(type(self.data), BaseModel):
                result.append(self.data.model_dump())
            else:
