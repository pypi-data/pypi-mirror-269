...
class HHRU:
    def __init__(
        self, login: str, password: str, file_session: str = "session.bin"
    ) -> None:
        """
    ...
    def minimum_time_bump(self):
        ...
        near_bump_time = int(
            (resumes[0].get("updated") + resumes[0].get("update_timeout", 14400000))
            / 1000
        )
    ...
    def ping_request(self):
        ...
        if self.session:
            for _ in range(self.count_requests):
                ...
                print(
                    "ПИНГ: Запрос прошел неудачно, попробую еще раз через %i секунд"
                    % (rnd)
                )
