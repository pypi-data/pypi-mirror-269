...
class ShareHandler(BaseHandler):
    ...
    async def get(self):
        if not self.current_user:
            ...
            docs = await self.query('share', {'uid': self.current_user.id})
            entries = []
            for doc in docs:
                entries.append(Dict({
                    'key': doc._id,
...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    async def get_info(self, path):
        ...
        if self.app.options.auth and not path.startswith('public'):
            doc = await self.db.share.find_one({'path': path, 'uid': self.current_user.id})
    ...
    async def check(self, name):
        ...
        if not uid.isdigit():
            return False
        doc = await self.db.share.find_one({'path': name})
        if not doc and self.args.key:
    ...
    async def post(self, name):
        ...
        if not path.exists() and self.args.action and self.args.action not in ['unshare', 'merge']:
            ...
            doc = await self.db.share.find_one_and_update({'uid': self.current_user.id, 'path': name},
                                                          {'$set': doc},
                                                          upsert=True,
    ...
    async def delete(self, name):
        ...
        if self.app.options.auth:
            await self.db.share.delete_many({'path': name})
