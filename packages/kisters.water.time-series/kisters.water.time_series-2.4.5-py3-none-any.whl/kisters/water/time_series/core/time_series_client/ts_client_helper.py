        ...
        if isinstance(paths, str):
            if (t0_start is None or isinstance(t0_start, datetime)) and (
                ...
                raise TimeSeriesUserError(
                    "When providing a single path, t0_start and t0_end cannot be iterables", paths
                )
        else:
            errors = []
            ...
            for key, ensemble_list in zip(
                ...
                await self._bulk_concurrent(
                    awaitables=[
                        self._read_ensemble_members(
                            path=path, t0_start=t0_start_i, t0_end=t0_end_i, **kwargs
                        )
        ...
        **kwargs: Any,
        ...
        if isinstance(keys, (TimeSeriesKey, str)):
            ...
            if (start is None or isinstance(start, datetime)) and (
                ...
                raise TimeSeriesUserError(
                    "When providing a single time series key or path, start and end cannot be iterables",
                    key,
                )
        else:
