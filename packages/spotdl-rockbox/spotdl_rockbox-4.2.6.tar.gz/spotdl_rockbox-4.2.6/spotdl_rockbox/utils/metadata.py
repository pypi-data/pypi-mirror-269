...
def embed_lyrics(audio_file, song: Song, encoding: str):
    ...
    if len(lrc_lines) == 0:
        ...
        # Embed them normally
        if encoding == "mp3":
            audio_file.add(USLT(encoding=Encoding.UTF16, text=song.lyrics))
        else:
...
def embed_wav_file(output_file: Path, song: Song):
    ...
    if song.cover_url:
        try:
            ...
            audio.tags.add(  # type: ignore
                APIC(
                    encoding=0, mime="image/jpeg", type=3, desc="Cover", data=cover_data
                )
            )
        except Exception:
            pass
    ...
    if song.lyrics:
        ...
        if len(lrc_lines) == 0:
            audio.tags.add(USLT(encoding=Encoding.UTF16, text=song.lyrics))  # type: ignore
        else:
            lrc_data = []
