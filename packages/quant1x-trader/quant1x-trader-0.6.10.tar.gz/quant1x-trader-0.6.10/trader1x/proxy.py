...
async def query_holding():
    ...
    查询当前持仓(√)
    """
    positions = __trader.query_positions()
    holding = []
    for position in positions:
        holding.append(
            {'account_type': position.account_type,
             ...
             'open_price': base1x.fix_float(position.open_price),
             'market_value': base1x.fix_float(position.market_value),
             'frozen_volume': position.frozen_volume,
             'on_road_volume': position.on_road_volume,
             'yesterday_volume': position.yesterday_volume,
             'avg_price': base1x.fix_float(position.avg_price),
             }
        )
    return holding
