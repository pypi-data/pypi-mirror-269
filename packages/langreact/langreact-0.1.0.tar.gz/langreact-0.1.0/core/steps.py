...
from core.common.chunk import NONE_CHUNK, Chunk, new_none_chunk
...
from core.tools import get_timer, is_sub_match
...
class PlanStep(Agent):
    ...
    async def __invoke__(
        ...
        for idx, title, desc in configure.PLAN_STEP_REGEX.findall(llm_answer.data):
            selected_events.append(Event(title, description=desc, prefer_score=1 / idx))
