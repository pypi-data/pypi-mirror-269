...
from ebs.linuxnode.mq.amqp import PikaService
from ..exceptions import WaitingForConnectionError
from ..base import ModularApiEngineBase


class AMQPCoreEngine(ModularApiEngineBase):
    _api_probe = '_probe'
    _api_announce = None
    _api_tasks = []
    ...
    def create_bindings(self):
        self.log.debug("Creating MQ Bindings")
        self.amqp = self._service.getFactory()

    @inlineCallbacks
    def start_service(self):
        parameters = yield self.connection_parameters()
        self._service = PikaService(parameter=parameters, postfix=self._prefix, connectionCb=self.handleConnectionState)
