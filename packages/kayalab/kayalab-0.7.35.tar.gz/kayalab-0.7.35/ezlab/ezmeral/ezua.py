...
def precheck():
    ...
    for out in execute(f"{ezfabricctl_cmd} pc -i ./tmp/prechecksInput.yaml -s ./tmp/prechecksStatus.txt"):
...
def install():
    ...
    for out in execute(
        f"{ezfabricctl_cmd} o init -p ./tmp/ezfab-release.tgz -i ./tmp/ezkf-input.yaml -s ./tmp/ezkf-orch-status.txt --save-kubeconfig ./tmp/mgmt-kubeconfig"
    ):
    ...
    for out in execute(f"{ezfabricctl_cmd} ph i -i ./tmp/hostPoolConfig.yaml -c ./tmp/mgmt-kubeconfig -s ./tmp/hostPoolConfigStatus.txt"):
        logger.info(out)

    write_clusterConfig_yaml()

    for out in execute(f"{ezfabricctl_cmd} w i -i ./tmp/clusterConfig.yaml -c ./tmp/mgmt-kubeconfig -s ./tmp/clusterConfigStatus.txt"):
        logger.info(out)

    for out in execute(
        f"{ezfabricctl_cmd} w g k -n ezlab -i ./tmp/clusterConfig.yaml -c ./tmp/mgmt-kubeconfig -s ./tmp/clusterConfigStatus.txt --save-kubeconfig ./tmp/workload-kubeconfig"
    ):
        logger.info(out)
...
def deploy():
    ...
    for out in execute(f"{kubectl_cmd} --kubeconfig=./tmp/workload-kubeconfig apply -f ./tmp/ezkfWorkloadDeploy.yaml"):
        logger.info(out)
        if "ezkfworkloaddeploy.ezkfops.hpe.ezkf-ops.com/ezlab" in out and any(i in out for i in ["created", "configured", "unchanged"]):
            ...
            logger.info(
                ...
                kubectl --kubeconfig=./tmp/workload-kubeconfig get ezkfworkloaddeploy/ezlab -n ezlab -o json | jq '.status.genericaddonsstatus | .[] | select(.installstatus == "INSTALLING")'
                """
            )
...
{hosts_yaml}"""
    ...
    with open("./tmp/prechecksInput.yaml", "wt") as f:
        f.write(input)
...
"""
    with open("./tmp/ezkf-input.yaml", "wt") as f:
        f.write(input)
...
"""
    with open("./tmp/hostPoolConfig.yaml", "wt") as f:
        f.write(input)
...
"""
    ...
    with open("./tmp/clusterConfig.yaml", "wt") as f:
        f.write(input)
...
spec:
  ...
  workloadtype: ezua
  clustername: ezlab
  domainname: ez.{app.storage.general["config"]["domain"]}
  isAirgap: false
  deployallinfra: true
  genericaddons:
    machine: true
...
"""
    ...
    with open("./tmp/ezkfWorkloadDeploy.yaml", "wt") as f:
        f.write(input)
