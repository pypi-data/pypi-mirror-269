...
#        self.from_labels = config_manager.get_from_csv_labels()
        self.to_labels = config_manager.get_to_csv_labels()
#        self.csv_from_primary_property = config_manager.get_csv_from_property_list()
            ...
            try:
                ...
                try:
                    with open(output_file+'.csv', 'w', encoding='UTF8', newline='') as f:
                        ...
                        writer.writeheader()
                        writer.writerows(self.all_nodes)
#                    print(f'Written {len(self.all_nodes)} lines to {output_file}.csv @2')
                except:
                    pass

## json            with open(output_file + '.json', 'w') as f:
## json                json.dump(self.all_nodes, f)
...
## json            with open(output_file + '.json', 'w') as f:
## json                json.dump(self.all_relationships, f)
    ...
    def update_node_data(self, data):
        ...
        for node in data:
            ...
            # This is needed for json file output.
            # Can simply use node['Properties'] if json support not implemented
## json            unchecked_properties = node['Properties']
## json            properties = {}
## json            for k, v in unchecked_properties.items():
## json                if isinstance(v, neo4j.time.Date):
## json                    properties[k] = str(v)
## json                elif isinstance(v, neo4j.time.DateTime):
## json                    properties[k] = str(v)
## json                else:
## json                    properties[k] = v

            properties = node['Properties']
            properties[self.node_label] = ':'.join(node['FromLabels'])

            self.pop_property(properties, self.config_manager.get_update_time_property())
#            self.pop_property(properties, self.config_manager.get_primary_id_property())
...
#                all_keys = list(set([*all_keys, self.primary_property]))
...
## json                for k, v in properties.items():
## json                    if isinstance(v, neo4j.time.Date):
## json                        properties[k] = str(v)
## json                    if isinstance(v, neo4j.time.DateTime):
## json                        properties[k] = str(v)

                properties[self.from_alias] = relationship['From']
                properties[self.to_alias] = self.to_prefix + relationship['To']
                properties[self.from_label] = ':'.join(relationship['FromLabels'])
                properties[self.to_label] = ':'.join(relationship['ToLabels'])
    ...
    def make_base_queries(self, labels, to_labels):
        ...
        where_literal = f'NOT {node_ref}:' + f' AND NOT {node_ref}:'.join(excl_labels)
        to_where_literal = f'NOT {node_ref}:' + f' AND NOT {node_ref}:'.join(to_excl_labels)
...
#                             f'{node_ref}.{self.csv_node_primary_property} AS From, ' \
    ...
    def process_data(self, node_list, label_set, to_label_set):
        ...
        while batch_num < num_batches:
            num_none = 0
            message_no_cr(f'Processing batch {batch_num+1} of {num_batches} at {assetmodelutilities.get_formatted_time_now_noms()}{chr(13)}')
...
#                    print(this_query_text)
    ...
    def write_csv_files(self):
        if self.write_node_file or self.write_relationship_file:
            node_labels_sets = self.combine_labels(self.node_labels)
            to_label_sets = self.combine_labels(self.to_labels)
            for labels in node_labels_sets:
                print('Fetching node list...')
                node_list = self.get_node_list(labels)
                print(f'There are {len(node_list)} nodes with {labels[0]}', end='')
                if labels[1]:
                    print(f' but not {labels[1]}')
                else:
                    print()
                self.process_data(node_list, labels, to_label_sets[0])
