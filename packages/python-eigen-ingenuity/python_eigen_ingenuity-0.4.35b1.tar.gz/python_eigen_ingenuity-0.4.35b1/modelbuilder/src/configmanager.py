...
class ConfigManager():
    ...
    def __init__(self):
        ...
        parser.add_argument('-ab', '--allowblanks', action='store_true', help='Update a property even if it is blank')
        ...
        parser.add_argument('-pp', '--primaryproperty', default='', help='Primary property')
        ...
        parser.add_argument('-np', '--nodeproperties', default=None, help='Primary property of nodes')
...
#        parser.add_argument('-cfl', '--csvfromlabels', nargs='+', default='', help='Lists of labels to identify from nodes to write to csv file')
        parser.add_argument('-ctl', '--csvtolabels', default='', help='Lists of labels to identify to nodes to write to csv file')
#        parser.add_argument('-cfp', '--csvfromproperties', nargs='+', default=None, help='Primary property of From nodes')
        parser.add_argument('-ctp', '--csvtoproperties', nargs='+', default=None, help='Primary property of To nodes')
        ...
        if labels_to_delete != '':
            self.labels_to_delete = [amu.validate_labels_and_relationships([i.replace(':', ',')]) for i in labels_to_delete]
        else:
...
##            settings.set_azure_tenant_id('74efa022-e6d6-42ad-bc15-edc76c525bdf')
        ...
        self.node_labels = amu.validate_labels_and_relationships([args.nodelabels])
        self.node_property_list = args.nodeproperties
        self.csv_file_suffix = args.csvfilesuffix
        self.relationship_csvfile_requested = args.createrelationshipcsvfile

#        self.csv_from_labels = args.csvfromlabels
        self.csv_to_labels = amu.validate_labels_and_relationships([args.csvtolabels])
#        self.csv_from_property_list = args.csvfromproperties
        self.csv_to_property_list = args.csvtoproperties
    ...
    def read_db_config(self):
        ...
        if not (database_num is None):
            ...
            if database_num < len(database_name_list):
                ...
                self.error_list.append(f'Database {self.db_name} requested, but only {len(database_name_list)} database(s) configured')
        else:
            # Must have been a name, so try to find it in the config
            try:
                database_index = database_name_list.index(self.db_name)
            except:
                self.error_list.append(f'Database {self.db_name} not configured')
    ...
    def read_version_config(self):
        ...
        if version_mode.lower() == 'on' and not self.no_version:
            self.frozen_labels_list = self.get_config_value_list("Versions", "FrozenLabels", "")
            self.version_labels_list = self.get_config_value_list("Versions", "VersionLabels", "!Current,Version")
            self.version_prefix = self.get_config_value("Versions", "VersionLabelPrefix", "")
            self.version_counter = self.get_config_value("Versions", "VersionCounterProperty", "versionCount")
            self.version_number = self.get_config_value("Versions", "VersionNumberProperty", "versionNumber")
            self.first_version_property = self.get_config_value("Versions", "FirstVersionProperty", "firstVersion")
            self.last_version_property = self.get_config_value("Versions", "LastVersionProperty", "lastVersion")
            self.version_relationship = self.get_config_value("Versions", "VersionRelationship", "has_version")
            self.next_version_relationship = self.get_config_value("Versions", "NextVersionRelationship", "next_version")
            self.version_valid_from_property = self.get_config_value("Versions", "VersionValidFromProperty", "validFrom")
            self.version_valid_to_property = self.get_config_value("Versions", "VersionValidToProperty", "validTo")
        else:
            self.no_version = True

    def read_csv_config(self):
        self.primary_property = self.primary_property_override or self.get_config_value("Model", "PrimaryProperty", "node")
#        config_labels = amu.validate_labels_and_relationships(self.get_config_value_list("Model", "RequiredLabels", ""))
#        self.required_labels_list = amu.validate_labels_and_relationships(config_labels)[1:].split(':')
        self.required_labels_list = amu.combine_required_labels(self.get_config_value_list("Model", "RequiredLabels", ""), [self.required_labels_override])

#        required_properties = self.get_config_value_list("Model", "RequiredProperties", "")
#        required_properties, unwanted_properties = amu.combine_required_properties(self.get_config_value_list("Model", "RequiredProperties", ""), [self.required_properties_override])
#        self.required_properties_dict = {}
#        try:
#            self.required_properties_dict = {amu.validate_property(i.replace('=', ':').split(':')[0].rstrip()): i.replace('=', ':').split(':')[1].lstrip() for i in required_properties if not i.startswith('!')}
#        except:
#            pass
#        self.unrequired_properties_list = [amu.validate_property(i.replace('!', '')) for i in required_properties if i.startswith('!')]

        self.required_properties_dict, self.unrequired_properties_list = amu.combine_required_properties(self.get_config_value_list("Model", "RequiredProperties", ""), [self.required_properties_override])
        ...
        self.unique_id_property = self.get_config_value("Model", "UniqueIDProperty", "uuid")
        self.creation_time_property = self.get_config_value("Model", "CreationTime", "creationTime")
        self.update_time_property = self.get_config_value("Model", "UpdateTime", "updateTime")
        header_mappings = self.get_config_value_list("Model", "Mappings", "")
        try:
            self.header_mappings_dict = {i.replace('=', '->').split('->')[0].rstrip(): i.replace('=', '->').split('->')[1].lstrip() for i in header_mappings}
        except:
            self.header_mappings_dict = {}
        self.default_data_type = self.get_config_value("Model", "DataType", "")

        self.label_alias_list = self.get_config_value_list("Aliases", "Labels", "label")
        self.node_alias_list = self.get_config_value_list("Aliases", "Nodes", "node") + [self.primary_property]
        self.from_alias_list = self.get_config_value_list("Aliases", "FromNodes", "from,start")
        self.from_label_alias_list = self.get_config_value_list("Aliases", "FromLabels", "fromLabel")
        self.to_alias_list = self.get_config_value_list("Aliases", "ToNodes", "to,end")
        self.to_label_alias_list = self.get_config_value_list("Aliases", "ToLabels", "toLabel")
        self.relationship_alias_list = self.get_config_value_list("Aliases", "Relationships", "relationship")

    def get_error_list(self):
        return self.error_list

    def get_config_value_list(self, section, option, default=None):
        # Get a value from the settings file, reporting an error if it's missing
        try:
            ...
            if default is not None:
                ...
                self.error_list.append(f'No value found for {option} in the configuration file')
                raw_value = ''
    ...
    def get_header_mappings(self):
        return self.header_mappings_dict
...
#    def get_from_csv_labels(self):
#        return self.csv_from_labels
#
    def get_to_csv_labels(self):
        return self.csv_to_labels

#    def get_csv_from_property_list(self):
#        return self.csv_from_property_list or self.primary_property
#
