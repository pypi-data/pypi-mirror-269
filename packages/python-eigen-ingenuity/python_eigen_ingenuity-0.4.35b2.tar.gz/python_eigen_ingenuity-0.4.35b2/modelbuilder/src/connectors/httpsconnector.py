...
class HTTPSConnector(GenericConnector):
    ...
    def convert_result(self, this_result, code=-1):
        ...
        json_result = None
        try:
#            json_result = json.loads(this_result.text)
        ...
        while not all_good and try_count < 3:
            ...
            try:
                ...
                if status_code != 200:
                    ...
                    all_good = False
                    if status_code == 414:
                        info_message(f'Query {self.query_id} failed at {assetmodelutilities.get_formatted_time_now_noms()} - data too long!')
                        break
                    elif status_code == 505:
                        info_message(f'Query {self.query_id} failed at {assetmodelutilities.get_formatted_time_now_noms()} - returned "ERROR"')
                        break
                    elif status_code == 401:
                        info_message(f'Query {self.query_id} failed at {assetmodelutilities.get_formatted_time_now_noms()} - Missing Required Azure Auth Credentials')
                        exit()
                    elif status_code != 600:
                        info_message(f'Query {self.query_id} failed at {assetmodelutilities.get_formatted_time_now_noms()} with error code {status_code}, retrying...')
                        sleep(2)
