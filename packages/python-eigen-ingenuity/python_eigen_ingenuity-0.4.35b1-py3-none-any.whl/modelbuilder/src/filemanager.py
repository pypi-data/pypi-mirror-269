...
from fileprocessors import FileProcessor
...
## json            case 'json':
## json                self.analyse_json_file(a_file)
            case 'cypher' | 'qry':
    ...
    def add_file_to_process_list(self, file_processor, file_type):
        ...
        self.valid_file_list[priority].append({'processor': file_processor, 'type': file_type})
    ...
    def analyse_files(self):
        ...
        for a_query in preprocess_list:
            if a_query is not None:
                ...
                if file_type == 'cypher' or file_type == 'qry':
                    this_file_processor = FileProcessor(a_query, self.config_manager)
                    self.add_file_to_process_list(this_file_processor, 'pre-file')
                else:
                    query_processor = FileProcessor(a_query, self.config_manager, True)
        ...
        if labels_to_delete_list == []:
            ...
            query_processor = FileProcessor(query, self.config_manager, True)
            self.add_file_to_process_list(query_processor, 'pre')
        else:
            for label_list in labels_to_delete_list:
                match_list = ''
                no_match_list = ''
                for label in label_list[1:].split(':'):
                    if label.startswith('!'):
    ...
    def analyse_csv_file(self, csv_file):
        # Analyse each of the specified input .csv files to determine if they contain Nodes or Relationships
        # If the file doesn't have a valid combination of columns (or is missing) they are placed in the appropriate list
        this_csv_file_processor = FileProcessor(csv_file, self.config_manager)
        this_csv_file_type, file_errors = this_csv_file_processor.determine_file_type('csv')
        self.add_file_to_list(this_csv_file_processor, this_csv_file_type, file_errors)

    def analyse_json_file(self, json_file):
        # Analyse each of the specified input .json files to determine if they contain Nodes or Relationships
        # If the file doesn't have a valid combination of columns (or is missing) they are placed in the appropriate list
        this_json_file_processor = FileProcessor(json_file, self.config_manager)
        this_json_file_type, file_errors = this_json_file_processor.determine_file_type('json')
        self.add_file_to_list(this_json_file_processor, this_json_file_type, file_errors)

    def analyse_cypher_file(self, query_file):
        this_file_processor = FileProcessor(query_file, self.config_manager)
                ...
                match file['type']:
                    case 'n':
                        if self.batch_rate == 0:
                            queries, file_result, version_result = self.process_file(processor, FileProcessor.process_node_file, db_connector)
                        else:
                            queries, file_result, version_result = self.process_file(processor, FileProcessor.process_node_file_batch, db_connector)
                    case 'r':
                        if self.batch_rate == 0:
                            queries, file_result, version_result = self.process_file(processor, FileProcessor.process_relationship_file, db_connector)
                        else:
                            queries, file_result, version_result = self.process_file(processor, FileProcessor.process_relationship_file_batch, db_connector)
                    case 'c' | 'pre-file' | 'post-file':
                        queries, file_result, version_result = self.process_file(processor, FileProcessor.process_cypher_file, db_connector)
                    case 'q' | 'pre' | 'post':
                        queries, file_result, version_result = self.process_file(processor, FileProcessor.process_query_list, db_connector)
                    case _:
    ...
    def process_file(self, file_to_process, file_processor, db_connector):
        ...
        queries = file_processor(file_to_process, self.start_from)
