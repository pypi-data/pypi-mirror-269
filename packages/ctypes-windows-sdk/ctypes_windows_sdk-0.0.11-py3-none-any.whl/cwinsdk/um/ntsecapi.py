...
from .winnt import PSID, ACCESS_MASK
from .lsalookup import LSA_UNICODE_STRING
from .. import windll, CEnum
from ctypes.wintypes import ULONG, LARGE_INTEGER, BOOLEAN
...
class _ForestTrustData(Union):
    _fields_ = [("TopLevelName", LSA_UNICODE_STRING),
...
LsaQueryTrustedDomainInfoByName.argtypes = [LSA_HANDLE, POINTER(LSA_UNICODE_STRING), TRUSTED_INFORMATION_CLASS, POINTER(PVOID)]
...
LsaEnumerateTrustedDomainsEx.argtypes = [LSA_HANDLE, POINTER(LSA_ENUMERATION_HANDLE), POINTER(PVOID), ULONG, POINTER(ULONG)]
LsaEnumerateTrustedDomainsEx.restype = NTSTATUS

LsaCreateTrustedDomainEx = windll.advapi32.LsaCreateTrustedDomainEx
LsaCreateTrustedDomainEx.argtypes = [LSA_HANDLE, POINTER(TRUSTED_DOMAIN_INFORMATION_EX), POINTER(TRUSTED_DOMAIN_AUTH_INFORMATION), ACCESS_MASK, POINTER(LSA_HANDLE)]
LsaCreateTrustedDomainEx.restype = NTSTATUS

LsaQueryForestTrustInformation = windll.advapi32.LsaQueryForestTrustInformation
LsaQueryForestTrustInformation.argtypes = [LSA_HANDLE, POINTER(LSA_UNICODE_STRING), POINTER(POINTER(LSA_FOREST_TRUST_INFORMATION))]
LsaQueryForestTrustInformation.restype = NTSTATUS

LsaSetForestTrustInformation = windll.advapi32.LsaSetForestTrustInformation
LsaSetForestTrustInformation.argtypes = [LSA_HANDLE, POINTER(LSA_UNICODE_STRING), POINTER(LSA_FOREST_TRUST_INFORMATION), BOOLEAN, POINTER(POINTER(LSA_FOREST_TRUST_COLLISION_INFORMATION))]
