...
def shared_awkward(arr: ak.Array, smm: SharedMemoryManager):
...
def from_shared(form, size, shared_buffers):
...
def do_write(args):
    ...
    del packed_slice
    return WriteResult(out_file_path=out_file_path, num_sequences=num_sequences, num_tokens=num_tokens)


def write_split_index(out_path: Path, chunk_results: list[WriteResult]) -> None:
    d = {
...
def shard_file(in_path: Path | str, out_path: str = None):
    ...
    with SharedMemoryManager() as smm:
        global shared_arr
        shared_arr = shared_awkward(arr, smm)
        pool = Pool(32)

        print(f'Starting mapping...')
        results = []
        for res in tqdm(pool.imap(do_write, [(i, out_path) for i in range(n_splits)]), total=n_splits, desc='Sharding dataset'):
            results.append(res)
