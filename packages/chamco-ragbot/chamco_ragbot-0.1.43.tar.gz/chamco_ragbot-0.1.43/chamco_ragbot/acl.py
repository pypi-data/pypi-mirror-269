...
storage_account_name = os.getenv('STORAGE_ACCOUNT_NAME', "datalakechamcorag")
storage_account_key = os.getenv('STORAGE_ACCOUNT_KEY', "VUuFEeVBkXi1hHpIusMcGrWXtggm30WXgtOfQtXBDHEFepQ2zA+mVlewgNU7s60YVbgnnp9vykiZ+AStj5mmIg==")
...
#             updated_file_metadata['group_ids'].append(department_group_id)
#     else:
#         # updated_file_metadata['group_ids'] = str(department_group_id)
#         updated_file_metadata['group_ids'] = department_group_id
...
def get_updated_file_metadata(file_metadata, department_group_id):
    ...
    # Extract just the first part of each group ID
    updated_group_ids = [group_id.split('-')[0] for group_id in updated_file_metadata['group_ids'].split(',')]
    updated_file_metadata['group_ids'] = ', '.join(updated_group_ids)
...
#     if 'group_ids' in file_metadata:
#         existing_group_ids = file_metadata['group_ids']
#         if isinstance(department_group_id, list):
#             for group_id in department_group_id:
#                 if group_id not in existing_group_ids:
#                     existing_group_ids.append(group_id)
#         elif department_group_id not in existing_group_ids:
#             existing_group_ids.append(department_group_id)
#     else:
#         if isinstance(department_group_id, list):
#             updated_file_metadata['group_ids'] = department_group_id
#         else:
...
def update_file_metadata(file_name, dept_name, blob_container_name):
    ...
    # updated_file_metadata['group_ids'] = [s.strip() for s in updated_file_metadata['group_ids'].split(',')]
    ...
    return updated_file_metadata, other_groups, file_metadata
