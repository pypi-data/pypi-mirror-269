"""The :class:`.ButterworthFiltersMixin` implements a mixin that can be added to :class:`.Signal` derived classes to
...
.. figure:: ../../.assets/img/002_example_butterworth_signal.png
    :width: 600
    :align: center
...
class ButterworthFiltersMixin(FiltersMixin):
    """Mixin derived from :class:`.FiltersMixin` for :class:`.Signal` that implements Butterworth filters."""
    ...
    def filters_butterworth(
        ...
        """Runs a Butterworth filter on the signal, refer to :func:`scipy.signal.butter` for more information.
        ...
        if all([lp_cutoff is not None, hp_cutoff is None, bp_cutoff is None, bs_cutoff is None]):
            ...
            raise FiltersError("Invalid cutoff frequency combination given to Butterworth filter.")
        ...
        if sos is None:
            raise FiltersError("Invalid Butterworth filter configuration given.")
