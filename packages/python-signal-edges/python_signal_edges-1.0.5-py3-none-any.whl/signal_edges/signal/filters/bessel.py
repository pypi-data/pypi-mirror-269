"""The :class:`.BesselFiltersMixin` implements a mixin that can be added to :class:`.Signal` derived classes to
...
.. figure:: ../../.assets/img/001_example_bessel_signal.png
    :width: 600
    :align: center
...
class BesselFiltersMixin(FiltersMixin):
    """Mixin derived from :class:`.FiltersMixin` for :class:`.Signal` that implements Bessel filters."""
    ...
    def filters_bessel(
        ...
        """Runs a Bessel filter on the signal, refer to :func:`scipy.signal.bessel` for more information.
        ...
        if all([lp_cutoff is not None, hp_cutoff is None, bp_cutoff is None, bs_cutoff is None]):
            ...
            raise FiltersError("Invalid cutoff frequency combination given to Bessel filter.")
        ...
        if sos is None:
            raise FiltersError("Invalid Bessel filter configuration given.")
