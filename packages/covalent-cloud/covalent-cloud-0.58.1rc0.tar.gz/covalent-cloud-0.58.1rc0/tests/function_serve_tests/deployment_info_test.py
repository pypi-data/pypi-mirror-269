...
from covalent_cloud.function_serve.models import DeploymentInfo, RouteInfo


class TestDeploymentInfo(unittest.IsolatedAsyncioTestCase):
    """Test the DeploymentInfo class."""
    ...
    def default_info(self):
        """Return a default DeploymentInfo object."""
        info = DeploymentInfo(
            function_id=self.deployment_id,
    ...
    def test_init_from_function_record(self):
        """Initialize a DeploymentInfo object with valid data."""
        ...
        info = DeploymentInfo.from_function_record(fake_function_record)
    ...
    def test_token_update(self):
        ...
        with patch(
            ...
            response = info.info()
            assert "Authorization" not in response["request_kwargs"]["headers"]

            # Set token and retry.
            info.token = "fake-token"
            response = info.info()
            assert response["request_kwargs"]["headers"]["Authorization"] == f"Bearer {info.token}"
...
class FakeResponse:
    ...
    def __init__(self, args, kwargs, info: DeploymentInfo, error=False):
