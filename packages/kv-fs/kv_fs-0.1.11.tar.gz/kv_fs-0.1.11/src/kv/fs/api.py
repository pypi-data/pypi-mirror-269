from typing import TypeVar, Generic, Callable, AsyncIterable, Never, Sequence
...
class FilesystemKV(LocatableKV[T], Generic[T]):
  ...
  @P.lift
  async def has(self, key: str) -> E.Right[Never, bool]:
    return Right(os.path.exists(self._path(key)))
  
  @P.lift
  async def keys(self) -> Right[Never, Sequence[str]]:
  ...
  @P.lift
  async def copy(self, key: str, to: 'KV[T]', to_key: str) -> E.Either[DBError|InexistentItem, None]:
  ...
  @P.lift
  async def move(self, key: str, to: 'KV[T]', to_key: str) -> E.Either[DBError|InexistentItem, None]:
    if not isinstance(to, FilesystemKV):
