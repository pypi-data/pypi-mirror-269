...
def test_list(mocker, requests_data, project_id, args, output):
    mock_metadata(mocker)
    mock_channel(mocker)
    stub = get_stub(mocker)
...
@mark.skip(reason="SetDataTtl not in public proto yet")
def test_set_data_ttl(mocker, oauth_token):
    ...
    args = Mock(token=oauth_token, id='job-id', days=40)

    set_data_ttl(args)

    stub.SetDataTtl.assert_called_once_with()
