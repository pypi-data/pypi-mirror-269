...
def MarkerRef2Tree(args):
    ...
    for gnm_id in gnm_id_list_sorted:
        for each_cog in marker_id_set:
            ...
            blast_cmd   = 'blastp -subject %s/%s.%s -evalue %s -outfmt 6 -max_target_seqs %s -query %s/%s.%s -out %s' % (marker_seq_dir, each_cog, marker_seq_ext, e_value, max_target_seqs, faa_file_dir, gnm_id, faa_file_ext, pwd_blast_op)
            if run_psiblast is True:
                blast_cmd = 'psiblast -subject %s/%s.%s -evalue %s -outfmt 6 -max_target_seqs %s -query %s/%s.%s -out %s' % (marker_seq_dir, each_cog, marker_seq_ext, e_value, max_target_seqs, faa_file_dir, gnm_id, faa_file_ext, pwd_blast_op)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mts',             required=False, default=500, type=float,    help='max_target_seqs, default: 500')
