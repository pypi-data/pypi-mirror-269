...
# des 10个词左右
@register(name="Relative Strength Index (RSI)", description="identify overbought or oversold")
def RSI(stock_id, **kwargs):
    try:
        ...
        return_info = "RSI: \n"
        df = {}
        df['days'] = get_data(stock_id, freq='days', n_periods=n_periods)[['date', 'RSI']]
        df['weeks'] = get_data(stock_id, freq='weeks', n_periods=n_periods)[['date', 'RSI']]

        for freq in ['days', 'weeks']:
            return_info += f"{freq}:{df[freq]['RSI'].round(1).tolist()}, " + "\n"

        return return_info
    except:
        return f"No RSI data"  
if __name__ == "__main__":
    result = RSI('000001', n_periods=30)
    print(result)
