...
async def add_testuser(db_session: AsyncSession):
    ...
    user = schemas.UserRegistrationSchema(username="testuser", password="P@ssw0rd#")
    await repo.create(user)


def test_generate_token_success(client: TestClient):
    payload = {"username": "testuser", "password": quote("P@ssw0rd#")}
...
def test_generate_token_fails_with_bad_username(client: TestClient):
    payload = {"username": "notvalid", "password": "P@ssw0rd#"}
    response = client.post("/api/auth/token", data=payload)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param({"password": "P@ssw0rd#"}, id="only password"),
