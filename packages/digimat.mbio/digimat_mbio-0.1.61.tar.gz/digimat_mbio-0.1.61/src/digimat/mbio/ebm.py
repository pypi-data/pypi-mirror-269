...
class MBIODeviceEBM(MBIODevice):
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        if item:
            ...
            if self.config.controlmode=='sensor':
                # sensor mode
                self.value('ai1', '%', commissionable=True)
                self.value('ai2', '%', commissionable=True)
                self.value('sp', unit='%', resolution=1)
            else:
                # speed mode
    ...
    def refresh(self):
        ...
        # TODO: refresh enable and sp if needed (not writable)
