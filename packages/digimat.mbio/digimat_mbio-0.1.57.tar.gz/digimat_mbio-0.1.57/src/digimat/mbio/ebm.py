...
class MBIODeviceEBM(MBIODevice):
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        if item:
            ...
            elif self.config.speedsource=='ai1':
                self.value('ai1', 'V', commissionable=True)
            elif self.config.speedsource=='ai2':
                self.value('ai2', 'V', commissionable=True)
            elif self.config.speedsource=='sensor':
                self.value('ai1', 'V', commissionable=True)
                self.value('ai2', 'V', commissionable=True)
