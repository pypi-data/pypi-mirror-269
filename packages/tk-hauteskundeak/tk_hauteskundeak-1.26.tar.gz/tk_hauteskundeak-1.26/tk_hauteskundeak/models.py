...
class Tokia(models.Model):
    ...
    def get_gurasoak(self):
        return self.herriak.filter(
            is_public=True, mota__gt=self.mota
        ).distinct()
...
class HauteskundeMota(models.Model):
    izena = models.CharField(max_length=255)
    slug = models.SlugField(db_index=True)
    akronimoa = models.CharField(max_length=5, null=True, blank=True)
    is_public = models.BooleanField(default=True, db_index=True)
    irudia = models.ForeignKey(
...
class HauteskundeaTokian(models.Model):
    ...
    eskrutinioa = models.DecimalField(
    ...
    def get_jarlekuak_sum(self):
        return self.hauteskundeemaitzaktokian_set.all().aggregate(
            Sum("jarlekuak")
        )["jarlekuak__sum"]
    ...
    def save(self, *args, **kwargs):
        if (
            ...
            self.alderdien_botoak = (
                self.boto_emaileak - self.baliogabeak - self.zuriak
            )
        for emai in HauteskundeEmaitzakTokian.objects.filter(
            hauteskundeatokian=self
        ):
            emai.save()
...
class HauteskundeEmaitzakTokian(models.Model):
    ...
    alderdia = models.ForeignKey(
        Alderdia, db_index=True, on_delete=models.CASCADE
    )
    ...
    for row1 in rd:
        ...
        if Tokia.objects.filter(
            ...
            HauteskundeaTokian.objects.filter(
                hauteskundea=hautes, tokia=tokia
            ).delete()
            ...
            hautesk_tok.boto_emaileak = (
                row1.pop("boto_emaileak").replace(".", "") or 0
            )
            hautesk_tok.baliogabeak = (
                row1.pop("baliogabeak").replace(".", "") or 0
            )
...
class HauteskundeaIframe(models.Model):
    ...
    izena = models.CharField(
        max_length=255, help_text="Sartu gogoratzeko moduko izena"
    )
    ...
    height = models.CharField(
        ...
        default="500px",
        help_text=(
            'Iframearen altuera. Sartu pixel kopurua: "400px", "600px"... '
        ),
    )
...
class APIResourceConfig(models.Model):
    ...
    path = models.CharField(
        max_length=255, verbose_name="Direktorioa/Helbidea"
    )
    filename = models.CharField(
        ...
        help_text=(
            "FTP edo HTTPS eskaeran fitxategi bat arkitu behar bada, adierazi"
            " hemen."
        ),
    )
