...
def _handle_failed_response_sync(response):
    if response.status_code == 401:
        raise ApiException(status_code=401, code="unknown", message="Unauthorized")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                _handle_failed_response_async(response)
...
def http_post(url, data=None, files=None):
    ...
    try:
        if files:
            ...
            if "stream" in data:
                response = httpx.post(url, json=data, stream=data["stream"])
            else:
