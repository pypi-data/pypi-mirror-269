...
class PlotRegr(CoreModule):
    ...
    def __init__(self, eventid, process="shakemap", logger=None):
        super(PlotRegr, self).__init__(eventid, logger=logger)
        if process == "shakemap":
...
def main():
    ...
    description = """
    Generate regression plots erom shake_results.hdf file.
    """
    ...
    if logdir is None:
        ...
        logfile = os.path.join(logdir, "plotregr.log")
    logger = get_generic_logger(logfile=logfile)

    mod = PlotRegr("noid", process="main", logger=logger)
