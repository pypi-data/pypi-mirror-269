...
class ModelModule(CoreModule):
    ...
    def execute(self, indir=None, outdir=None):
        ...
        try:
            ...
            self.rupture_obj._origin.mech = "ALL"
            self.rupture_obj._origin._tectonic_region = "unknown"
            self.rupture_obj._origin._slab_model_dip = 65.0
            self.rupture_obj._origin._slab_model_strike = 0.0
        else:
        ...
        if isinstance(self.rupture_obj, PointRupture):
            self.rff = RandomFiniteFault(self.rupture_obj._origin, 100)
        else:
            self.rff = None
        ...
        if isinstance(self.rupture_obj, PointRupture):
            ...
            for imt_str in self.imt_out_set:
                ...
                gmpes[oqimt] = gmpe
            ffsim = FFSimmer(self.rupture_obj._origin, self.rff)
            ...
            if self.do_grid:
                ...
                gsa = GenericSiteAmplification(
                    self.rupture_obj._origin._tectonic_region,
                ...
                shapeset = set(shapes)
                if len(shapeset) != 1:
                    raise Exception("All sites elements must have same shape.")
                else:
            ...
            for k, v in self.sx_out.__dict__.items():
                ...
                if (k != "lons") and (k != "lats"):
                    shapes.append(v.shape)
                    self.sx_out.__dict__[k] = np.reshape(self.sx_out.__dict__[k], (-1,))
            for k, v in self.dx_out.__dict__.items():
                if k == "_slots_":
                    continue
                if (k != "lons") and (k != "lats") and v is not None:
                    shapes.append(v.shape)
                    self.dx_out.__dict__[k] = np.reshape(self.dx_out.__dict__[k], (-1,))
            shapeset = set(shapes)
            if len(shapeset) != 1:
            ...
            for k, v in self.dx_out.__dict__.items():
                if k == "_slots_":
                    continue
                if (k != "lons") and (k != "lats") and v is not None:
                    self.dx_out.__dict__[k] = np.reshape(
                        self.dx_out.__dict__[k], orig_shape
                    )
            for k, v in self.sx_out.__dict__.items():
                if k == "_slots_":
                    continue
                if (k != "lons") and (k != "lats"):
                    self.sx_out.__dict__[k] = np.reshape(
                        self.sx_out.__dict__[k], orig_shape
                    )
        ...
        for imt_str in self.imt_out_set:
            ...
            self._computeMVN(imt_str, griddict[oqimt])
    ...
    def _populateDataFrames(self):
        ...
        for dfid in self.dataframes:
            ...
            pdict = {}
            if len(myimts) > 0:
                pdict = self.getPointAmps(gmpe, myimts, dfn)
            ...
            for imtstr in imt_set:
                ...
                if self.do_outliers and (
                    ...
                    self.logger.debug(f"IMT: {imtstr}, flagged: {np.sum(flagged)}")
    ...
    def getPointAmps(self, gmpe, imt_set, dfn):
        """
        Get predictions for site, rock, and soil for a set of locations for a
        list of IMTs.
        """
        if isinstance(self.rupture_obj, PointRupture):
            ffs = FFSimmer(self.rupture_obj._origin, self.rff)
            do_mmi = False
            immi = imt.MMI()
            if immi in imt_set:
                imt_set.remove(immi)
    ...
    def _deriveIMTsFromMMI(self):
        ...
        pdict = self.getPointAmps(gmpe, imtlist, self.df2)
    ...
    def _deriveMMIFromIMTs(self):
        ...
        pdict = self.getPointAmps(self.ipe, [immi], self.df1)
    ...
    def _computeBias(self):
        ...
        for imtstr in self.imt_out_set:
            ...
            if nsta == 0:
                ...
                #
                self.logger.debug(
                    f"{imtstr}: nom bias {self.nominal_bias[imtstr]} "
                    f"nom stddev {np.sqrt(nom_variance)}; "
                    f"{nsta} stations (time={bias_time} sec)"
                )
                continue
            #
    ...
    def _computeMVN(self, imtstr, ddict):
        """
