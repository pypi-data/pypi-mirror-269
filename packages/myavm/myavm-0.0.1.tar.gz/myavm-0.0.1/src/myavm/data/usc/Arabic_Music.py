...
def generate_Arabic_Music_folds(data_dir, output_dir, l3embedding_model=None,
                         features='l3', random_state=12345678, **feature_args):
    for fold_idx in range(NUM_FOLDS):
        generate_Arabic_Music_fold_data(data_dir, fold_idx, output_dir,
                                 l3embedding_model=l3embedding_model,
                                 features=features, random_state=random_state,
                                 **feature_args)


def generate_Arabic_Music_fold_data(data_dir, fold_idx, output_dir, l3embedding_model=None,
                             features='l3', random_state=12345678, **feature_args):
    # Set random seed
    ...
    for idx, f in enumerate(files):
        ...
        with LogTimer(LOGGER, desc, log_level=logging.DEBUG):
            generate_Arabic_Music_file_data(fname, audio_fold_dir, output_fold_dir,
                                     features, l3embedding_model, **feature_args)


def generate_Arabic_Music_file_data(fname, audio_fold_dir, output_fold_dir,
                             features, l3embedding_model, **feature_args):
    ...
    class_label = int(basename.split('_')[-1])
    y = class_label
