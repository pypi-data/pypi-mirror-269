...
def sample_one_second(audio_data, sampling_frequency, augment=False):
    ...
    if len(audio_data) > sampling_frequency:
        ...
        #print('start in sampl-on-second', start)
    else:
        start = 0
...
def sample_one_frame(video_data, start=None, fps=30, augment=False):
    ...
    if start is not None:
        ...
        #print('start in sample_on_frame', start_frame)
        ...
        #print('nb frames', num_frames)
        #print('nb frames minus start_frame', num_frames - start_frame)
        ...
        if duration > 0:
            ...
            # print('we will take finally the frame number', frame)
        else:
...
def data_generator(subset_path, k=32, batch_size=64, random_state=20171021,
    ...
    # mux = pescador.StochasticMux(seeds, k, rate=rate, random_state=random_state, mode='exhaustive')
    mux = pescador.StochasticMux(seeds, k, rate=rate, random_state=random_state)
    if cycle:
        mux = mux.cycle()
