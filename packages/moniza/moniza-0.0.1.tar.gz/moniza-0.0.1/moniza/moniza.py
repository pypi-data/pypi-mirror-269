...
# by Ohm Lim <jo.studio.dev@gmail.com>
...
ini_files = (
    os.path.join('/etc', 'moniza.ini'),
    os.path.join('/etc', 'moniza-custom.ini'),
    os.path.join('/etc', 'moniza-token.ini'),
    os.path.join(os.path.dirname(__FILEABSDIRNAME__), 'moniza.ini'),
    os.path.join(os.path.dirname(__FILEABSDIRNAME__), 'moniza-custom.ini'),
    os.path.join(os.path.dirname(__FILEABSDIRNAME__), 'moniza-token.ini'),
    os.path.abspath('moniza.ini'),
    os.path.abspath('moniza-custom.ini'),
    os.path.abspath('moniza-token.ini'),
)

if os.name == 'nt':
    ini_files = (
        os.path.join(__FILEABSDIRNAME__, '..', 'config', 'moniza.ini'),
        os.path.join(__FILEABSDIRNAME__, '..', 'config', 'moniza-custom.ini'),
        os.path.join(__FILEABSDIRNAME__, '..', 'config', 'moniza-token.ini'),
    )

def info():
    '''
    Return string with info about moniza:
        - version
        - plugins enabled
        - absolute path to plugin directory
        - server id from configuration file
    '''
    moniza = Moniza(dry_instance=True)
    plugins_path = moniza._get_plugins_path()

    plugins_enabled = moniza._get_plugins(state='enabled')

    return '\n'.join((
        'Version: %s' % __version__,
        'Plugins enabled: %s' % ', '.join(plugins_enabled),
        'Plugins directory: %s' % plugins_path,
        'Server: %s' % moniza.config.get('moniza', 'server'),
    ))
...
def test_plugins(plugins=[]):
    ...
    if not plugins:
        plugins = moniza._get_plugins(state='enabled')
    ...
    for plugin_name in plugins:
        ...
        try:
            payload = module.Plugin().run(moniza.config)
...
class Moniza:
    ...
    def _get_plugins_path(self):
        if os.name == 'nt':
            return os.path.expandvars(self.config.get('moniza', 'plugins'))
        else:
            return self.config.get('moniza', 'plugins')
    ...
    def _config_init(self):
        ...
        defaults = {
            ...
            'user': '',
            'server': '',
            'api_host': 'ingest.moniza.xyz',
            'api_path': '/v2/server/poll',
            'log_file': '/var/log/moniza.log',
            'log_file_mode': 'a',
    ...
    def _logging_init(self):
        ...
        '''
        level = self.config.getint('moniza', 'logging_level')

        if os.name == 'nt':
            log_file = os.path.expandvars(self.config.get('moniza', 'log_file'))
        else:
            log_file = self.config.get('moniza', 'log_file')

        log_file_mode = self.config.get('moniza', 'log_file_mode')
        ...
        logging.info('Moniza logging_level %i', level)
        ...
        while True:
            ...
            if isinstance(task, basestring):
                ...
                try:
                    ...
                    plugin = task.Plugin(moniza_cache=self.plugins_cache[name])
    ...
    def _data(self):
        ...
        max_age = self.config.getint('moniza', 'max_data_age')
        max_span = self.config.getint('moniza', 'max_data_span')
        server = self.config.get('moniza', 'server')
        user = self.config.get('moniza', 'user')
        interval = self.config.getint('data', 'interval')
        max_cached_collections = self.config.get('moniza', 'max_cached_collections')
        cached_collections = []
        collection = []
        initial_data = True
        while True:
                        ...
                        try:
                            ...
                            if cached_collections:
                                ...
                                while cached_collections:
                                    connection.request('POST', '%s?version=%s' % (api_path, __version__),
    ...
    def run(self):
        ...
        '''
        logging.info('Moniza main loop')
        interval = self.config.getfloat('moniza', 'interval')
...
def main():
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == 'help':
            print('\n'.join((
                'Run without options to run moniza.',
                'Acceptable options (leading -- is optional):',
                '    help, info, version, test',
            )))
            sys.exit()
