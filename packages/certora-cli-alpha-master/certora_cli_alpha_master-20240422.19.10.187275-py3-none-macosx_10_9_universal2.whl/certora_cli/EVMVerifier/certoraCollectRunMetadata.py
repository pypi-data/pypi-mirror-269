...
def collect_run_metadata(wd: Path, raw_args: List[str], raw_input_conf_dict: Dict[str, Any], context: CertoraContext) \
    ...
    if is_windows():
        return RunMetaData(raw_args, raw_input_conf_dict, "", "", "", wd, True, None)
    ...
    if not is_git_executable:
        ...
        return RunMetaData(raw_args, raw_input_conf_dict, "", "", "", improvise_cwd_relative(wd), True,
                           context.main_spec)
    ...
    try:
        ...
        data = RunMetaData(raw_args, raw_input_conf_dict, origin, sha, branch_name, cwd_relative, dirty,
                           context.main_spec)
