...
class GoogleContacts(GenericCRUD):
                ...
                try:
                    ...
                    for index, google_contact_connection in enumerate(google_connections):
                        ...
                        try:
                            ...
                            location_results = self.__insert_link_contact_location(contact_dict=contact_dict, contact_id=contact_id)
                            if location_results:
    ...
    def __insert_link_contact_location(self, contact_dict: dict, contact_id: int) -> list:
        logger.start(object={"contact_dict": contact_dict, "contact_id": contact_id})
        if not contact_dict.get("locations"):
            logger.end(log_message="contact_dict['locations'] is None")
            return None
    ...
    def __procces_location_of_google_contact(self, contact_dict: dict) -> dict:
        """
        Process location of Google contact
        :param location_dict: location_dict
        :return: location_dict
        """
        location_dicts = contact_dict.get("locations")
        ...
        if not location_dicts and not phone_numbers_list and not email_addresses_list:
            logger.end(log_message="location_dict is None and phone_numbers_list is None and email_addresses_list is None")
            return None
