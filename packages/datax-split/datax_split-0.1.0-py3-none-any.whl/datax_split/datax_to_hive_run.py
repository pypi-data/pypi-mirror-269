    ...
    try:
        if settings.RUN_TYPE in (0, 1):
            ...
            if settings.TYPE == 'all':
                ...
                where = f"{table.get('update_column', '')} between '{settings.start_time}' and '{settings.end_time}'"
            else:
                where = ""
...
def run():
    ...
    settings.logger.info(f'生成开始--type:{TYPE}--{settings.partition_date}')
    with concurrent.futures.ThreadPoolExecutor(max_workers=settings.max_workers) as executor:
        # 提交每个表的处理任务到线程池
