...
import spyctl.resources.rulesets as _rs
...
def merge_resource(
...
) -> Optional[List[m_lib.MergeObject]]:
    if target == with_obj:
        cli.try_log(
    ...
    resrc_kind = target.get(lib.KIND_FIELD)
    is_rs_merge = lib.RULESETS_FIELD in target[lib.SPEC_FIELD]
    if is_rs_merge:
        return list(
            merge_ruleset_policy(
                target,
                target_name,
                merge_with_objects,
                src_cmd,
...
def merge_ruleset_policy(
    ...
    for rs in rulesets:
        ...
        merge_obj = get_merge_object(lib.RULESET_KIND, rs, True, src_cmd)
        for w_obj in merge_with_objects:
            merge_obj.asymmetric_merge(w_obj, check_irrelevant)
        yield merge_obj
...
def get_merge_object(
    ...
    if resrc_kind == lib.BASELINE_KIND:
        ...
        merge_obj = m_lib.MergeObject(
            target, _rs.RULESET_MERGE_SCHEMAS, schemas.valid_object
        )
    else:
        cli.try_log(
