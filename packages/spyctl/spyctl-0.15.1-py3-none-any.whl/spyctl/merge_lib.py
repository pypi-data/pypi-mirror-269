...
class MergeObject:
    ...
    def __add_diff_highlights(self, diff_lines: List[str]):
        for i, line in enumerate(diff_lines):
            if line.startswith(ADD_START):
                diff_lines[i] = f"{lib.ADD_COLOR}{line}{lib.COLOR_END}"
            elif line.startswith(SUB_START):
...
class MergeSchema:
    def __init__(
        self,
        field: str,
        sub_schemas: Dict[str, "MergeSchema"] = None,
        merge_functions: Dict[str, Callable] = None,
...
class ProcessNodeList:
    ...
    def __add_node(
        ...
        if "policyNode" in node_data:
            ...
            if node_id not in BASE_NODE_LIST.ids:
                raise InvalidMergeError(
                    f"Deviation process node ID ({node_id}) is missing in base"
                    " policy"
                )
            node = BASE_NODE_LIST.get_node(node_id)
...
def list_diffs(
    ...
    # elif ancestor_fields[-1] == lib.RULES_FIELD:
    #     # We are dealing with ruleset rules
    #     diffs.append(
    #         OriginalLines(
    #             parent_index,
    #             parent_index + 1,
    ...
    #         )
    #     )
    #     orig_checksums = defaultdict(list)
    #     other_checksums = defaultdict(list)
    #     for i, orig_item in enumerate(original_data):
    #         checksum = lib.make_checksum(orig_item)
    #         orig_checksums[checksum].append(i)
    #     for i, other_item in enumerate(other_data):
    #         checksum = lib.make_checksum(other_item)
    #         other_checksums[checksum].append(i)
    #     for other_checksum, indexes in other_checksums.items():
    #         if other_checksum in orig_checksums:
    #             # The rule exists in both orig and new
    #             for i in indexes:
    #                 diffs.append(
    #                     OriginalLines(
    ...
    #                     )
    #                 )
    #     for orig_checksum in orig_checksums:
    #         if orig_checksum not in other_checksums:
    else:
        diffs.append(
            DiffLines(
                parent_index,
