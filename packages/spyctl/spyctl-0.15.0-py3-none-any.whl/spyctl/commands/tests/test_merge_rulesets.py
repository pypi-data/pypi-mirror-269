...
def test_merge_ruleset_policy(
    ...
    with mock.patch("spyctl.commands.merge.api.get_rulesets", get_rulesets):
        out_rs = list(
            _merge.merge_ruleset_policy(cluster_policy, policy_name, deviation)
        )[0]
        rs_dict = out_rs.get_obj_data()
        print(out_rs.get_diff(full_diff=False))
...
def test_merge_ruleset_policy_full_command(rulesets, updated_ruleset_string):
    ...
    with mock.patch(
        "spyctl.commands.merge.api.get_rulesets", get_rulesets
    ), mock.patch.object(
        Context,
        "get_api_data",
