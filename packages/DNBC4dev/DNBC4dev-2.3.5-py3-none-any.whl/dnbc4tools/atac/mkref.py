...
def write_config(genomeDir, species, gtf, chrM, chloroplast, blacklist, fasta,tag,prefix):
    ...
    if blacklist != 'None':
        if blacklist == 'hg19':
            ref_dict['blacklist'] = '%s/config/atac/blacklist/hg19.full.blacklist.bed'%__root_dir__
        elif blacklist == 'hg38':
            ref_dict['blacklist'] = '%s/config/atac/blacklist/hg38.full.blacklist.bed'%__root_dir__
        elif blacklist == 'mm10':
            ref_dict['blacklist'] = '%s/config/atac/blacklist/mm10.full.blacklist.bed'%__root_dir__
        else:
...
def helpInfo_mkref(parser):
    ...
    parser.add_argument(
        ...
        metavar='<CHLOROPLAST>',
        default='None',
        help='Chloroplast chromosome names, particularly recommended for plant, such as the name "Pt.", "chrP"'
        )
    parser.add_argument(
        '--blacklist',
        metavar='<BLACKLIST>',
