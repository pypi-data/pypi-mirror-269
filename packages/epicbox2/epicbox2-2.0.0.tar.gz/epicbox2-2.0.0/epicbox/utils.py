...
def read_write_socket(sock, stdin=None, timeout=None):
    """
    Interact with the container: Send data to stdin.
...
def docker_communicate(container, stdin=None, start_container=True, timeout=None):
    ...
    sock = get_socket(container)

    if start_container:
        container.start()
        log.info("Container started")

    return read_write_socket(sock, stdin=stdin, timeout=timeout)
