...
#                         video_quality['error'] = 'no_face_detected_in_video'
    ...
    def load_and_process_image_fr(self, base64_image, arr=False):
        # try:
            ...
            # print(f"FACE LOC: {face_locations}")
            if len(face_locations) == 0:
                print(f"going upsampling way")
                try:
                    ...
                    # print(f"FACE LOC upsampled: {face_locations}")
                except:
                    return [], []

            face_encodings = []
            face_encodings = face_recognition.face_encodings(image, face_locations)
            # print(f"FACE ENC: {face_encodings}")

            if len(face_encodings) == 0:
                print(f"going upsampling way")
                try:
                    face_encodings = face_recognition.face_encodings(image, face_locations, model="large")
                    # print(f"FACE ENC upsampled: {face_encodings}")
                except:
                    return [], []
