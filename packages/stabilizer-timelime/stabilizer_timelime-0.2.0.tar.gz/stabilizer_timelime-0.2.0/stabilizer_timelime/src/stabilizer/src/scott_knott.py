...
class Rx(Mine):
  ...
  def fileIn(f,lo=0,hi=1):
    ...
    for word in words(f):
       ...
       if isinstance(x,str): 
          ...
          d[what] += [x]
    return Rx.ret(Rx.sk(Rx.data(**d)),lo=lo,hi=hi)
  ...
  def show(rxs,lo=0,hi=1):
    ...
    for rx in sorted(rxs):
        qs = []
        ordered = sorted(rx.vals)
        for c in THE.tile.chops:
          qs.append(ordered[int(len(rx.vals)*c)])
        print(THE.rx.show % (rx.rank, rx.rx, 
             rx.tiles(lo=lo,hi=hi)))
    
  @staticmethod
  def ret(rxs,lo=0,hi=1):
    "pretty print set of treatments"
    tmp=Rx.sum(rxs)
    lo,hi=tmp.vals[0], tmp.vals[-1]
    result = []
    for rx in sorted(rxs):
        ...
        for c in THE.tile.chops:
          qs.append(ordered[int(len(rx.vals)*c)])
        r = [rx.rank, rx.rx, qs[2], qs[3]-qs[1], qs[0], qs[1], qs[3], qs[4]]
        result.append(r)
    return result
