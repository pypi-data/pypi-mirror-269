...
from flexible_semantic_kernel.connectors.ai import AIRequestSettings
...
from flexible_semantic_kernel.plugin_definition.read_only_plugin_collection import (
...
class Plan(SKFunctionBase):
    ...
    async def invoke_async(
        ...
        **kwargs,
        ...
        context.function_name = self.name
        if self.is_semantic or self.is_native:
            logger.info('{"is_finished": false, "type": "plan", "step": "action", "step_value": "%s.%s"}',
                        self.plugin_name, self.name)
            logger.info('{"is_finished": false, "type": "plan", "step": "action_input", "step_value": "%s"}',
                        context.result)
        ...
        if self.is_semantic or self.is_native:
            logger.info('{"is_finished": false, "type": "plan", "step": "action_output", "step_value": "%s"}',
                        context.result)
        return context
