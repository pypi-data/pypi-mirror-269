...
class large_tabs(ctk.CTkFrame):
    ...
    def row_frame(self):
        ...
        elif len(self.tabs) % 3 == 0:                           #todo: rewrite this check to see if the width of the new tab is gonna fit in the current row or does it need a new one
            row = ctk.CTkFrame(self, fg_color="transparent")
            row.pack(fill="x", expand=True)
        else:
            row = 0
    ...
    def tab(self, text=None, image=None, limage_height=250):  
        # im = Image.open(image)
        # w, h = im.size[0],im.size[1]
        # r = w/h
        # s = (int(limage_height*r), limage_height)
        # im_ctk = ImageTk.PhotoImage(im.resize(s))                

        row_getter = self.row_frame()
        self.parent_row = row_getter if row_getter!=0 else self.parent_row
        expander = ctk.CTkFrame(self.parent_row, fg_color="red", height=300)
        tab_cont = ctk.CTkFrame(expander, fg_color="blue", height=300)

        canvas = ctk.CTkCanvas(tab_cont, bg="yellow", bd=0, highlightthickness=0, relief='ridge')
        canvas.pack()

        content = ctk.CTkFrame(tab_cont, fg_color="green", width=canvas.winfo_width())
        text = ctk.CTkLabel(content, text=f" {text}", font=(FONT, 20), fg_color="purple", text_color=(LIGHT_MODE["text"], DARK_MODE["text"]))
        text.pack(side="left", pady=5)
        actbtn = ctk.CTkButton(content, text="Click", font=(FONT, 15), fg_color=(LIGHT_MODE["accent"], DARK_MODE["accent"]), hover_color=(hvr_clr_g(LIGHT_MODE["accent"], "l", 20), hvr_clr_g(DARK_MODE["accent"], "d", 20)), command= lambda : None)
        actbtn.pack(side="right")
        content.pack(expand=True, fill="x")
        
        tab_cont.pack()
        expander.pack(expand=True, fill="both", side="left", padx=10, pady=10)
