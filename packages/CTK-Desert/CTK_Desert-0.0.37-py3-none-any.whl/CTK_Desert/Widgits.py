...
class large_tabs(ctk.CTkFrame):
    ...
    def row_frame(self):
        ...
        elif self.parent_row.winfo_width() < (self.image_width+self.padx)*(self.tabs_counter+1):  
            self.tabs_counter = 0
            row = ctk.CTkFrame(self, fg_color="transparent")
            row.pack(fill="x", expand=True)
        else:
    ...
    def tab(self, text=None, image=None):  
        im = Image.open(image)
        w, h = im.size[0],im.size[1]
        r = w/h
        ...
        self.tabs_counter += 1
        expander = ctk.CTkFrame(self.parent_row, fg_color="transparent", height=300)
        ...
        content = ctk.CTkFrame(tab_cont, fg_color="grey", width=canvas.winfo_width())
        ...
        actbtn.pack(side="right", padx=10, pady=5)
        content.pack(expand=True, fill="x", pady=10)
        
        tab_cont.pack()
        expander.pack(expand=True, fill="both", side="left", padx=self.padx, pady=self.pady)
