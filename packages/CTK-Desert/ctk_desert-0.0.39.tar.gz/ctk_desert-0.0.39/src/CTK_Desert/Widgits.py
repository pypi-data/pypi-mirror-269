...
class large_tabs(ctk.CTkFrame):
    def __init__(self, page_class, parent, img_width=500, img_height=300, padx=10, pady=10):
    ...
    def row_frame(self):
        if len(self.tabs) == 0:
            ...
            self.tabs_per_row += 1
        elif self.rows[-1].winfo_width() < (self.image_width+self.padx)*(self.tabs_counter+1):  
            self.tabs_per_row = self.tabs_counter
            self.tabs_counter = 0
            row = ctk.CTkFrame(self, fg_color="transparent")
            row.pack(fill="x", expand=True)
        else:
            if len(self.tabs) == 1:
                self.tabs_per_row += 1  #* problem here
            row = 0

        return row

    def tab(self, text=None, image=None, button_icon=None, icon_size=20, button_command=None, autofit=True):  
        im = Image.open(image)
        w, h = im.size[0],im.size[1]
        r = w/h
        ...
        self.tabs_counter += 1
        expander = ctk.CTkFrame(self.rows[-1], fg_color="transparent", height=300)
        tab_cont = ctk.CTkFrame(expander, fg_color="transparent", height=300)
        ...
        tab_cont.pack()
        expander.pack(expand=autofit, fill="both", side="left", padx=self.padx, pady=self.pady)
        
        self.tabs.append(expander)
        self.page.called_when_opened()

    def update(self):
        self.available_tab_spaces = int(self.rows[-1].winfo_width() / (self.image_width+self.padx))
        # print("tabs i have:", len(self.tabs), " _  available spaces", self.available_tab_spaces, "\n --------------------------------------------------------")
        if len(self.tabs) >= self.available_tab_spaces:
            while True:
                # print("what i have in a row:", self.tabs_per_row)
                if self.tabs_per_row == self.available_tab_spaces:
                    break

                elif self.tabs_per_row < self.available_tab_spaces: #increase the number of tabs
                    # print("increase")
                    self.tabs_per_row += 1

                elif   self.tabs_per_row > self.available_tab_spaces:   #reduce the number of tabs
                    # print("reduce")
                    self.tabs_per_row -= 1
