...
from astroscript.save_event import load_event, load_location, save_event, save_location
...
# Copy the ephemeris files from the package to the temporary directory
with resources.path('astroscript.ephe', 'seas_18.se1') as eph_file:
    shutil.copy(eph_file, temp_dir)

# Set the path for Swiss Ephemeris to the temporary directory
swe.set_ephe_path(temp_dir)
...
# Set up paths for user-specific data storage
...
def get_coordinates(location_name:str):
    ...
    location_details = load_location('Sahlgrenska')
    if location_details:
...
def main(gui_arguments=None):    
    ...
    if name and not exists:
        new_data = {name: {"location": place,
                           ...
                           "latitude": latitude,
                           "longitude": longitude}}
        save_event(new_data.key(), new_data.values())
