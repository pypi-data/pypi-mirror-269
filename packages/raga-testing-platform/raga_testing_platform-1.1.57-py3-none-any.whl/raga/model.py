    ...
    def upload_file_to_s3(self, pre_signed_url, file_path):
        ...
        with tqdm(total=file_size, unit='B', unit_scale=True, desc=file_path.split('/')[-1]) as pbar:
            with open(file_path, "rb") as fd:
                for chunk in iter(lambda: fd.read(4096*10000), b""):  # Read the file in chunks
                    response = requests.put(pre_signed_url, data=chunk, headers={"Content-Type": "application/x-tar"})
