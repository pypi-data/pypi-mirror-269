...
'''
	...
	async def retry_hung_jobs(self):
		...
		db_items, db_err = await self.dbc.key_query(query_key = f"{self.batch_sql_base}.bump_batch_items", params=updated_status, mode = "none", item_type = dict)
		if db_err:
	...
	async def bump_job_by_id(self, id, status):
		...
		return await self.dbc.key_query(query_key = f"{self.batch_sql_base}.bump_batch_item_status", params=updated_status, mode = "none", item_type = dict)
