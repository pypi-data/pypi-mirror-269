...
##########################################################################
"""
Brief:FEATURE_AR_IMG_BODY_GPU_V1 Demo, 以及压测数据生成的DEMO
Author: liudongdong04(liudongdong04@baidu.com)
Date: 2021-01-21
Filename: FEATURE_AR_IMG_BODY_GPU_V1.py
"""
import os
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_AR_IMG_BODY_GPU_V1 demo
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_AR_IMG_BODY_GPU_V1',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_AR_IMG_BODY_GPU_V1.py
    生成压测数据：
        python FEATURE_AR_IMG_BODY_GPU_V1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测词表
        input_data = {
            'param': './image_dir/FEATURE_AR_IMG_BODY_GPU_V1/param',  # 参数配置目录
            'image': './image_dir/FEATURE_AR_IMG_BODY_GPU_V1/image'  # 图片目录
        }
        gen_stress_data(input_data)
    else:
        # 特征计算Demo
        input_data = {
            'image': './image_dir/FEATURE_AR_IMG_BODY_GPU_V1/image/boy.jpg'
        }
