...
@Filename: FEATURE_VIS_IMG_HUMANSEG_GPU_DUKA.py
"""
...
from proto.FEATURE_VIS_IMG_HUMANSEG_GPU_DUKA import parseservice_pb2
from proto.protobuf_to_dict import protobuf_to_dict


class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_IMG_HUMANSEG_GPU_DUKA demo
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_VIS_IMG_HUMANSEG_GPU_DUKA',
    ...
    # if 1: 解析res中foreground字段
    if 0:
        foreground = base64.b64decode(res_data["feature_result"]["value"]["result"]["foreground"])
        import numpy as np
        import cv2
        nparr = np.fromstring(foreground, np.uint8)
        foreground_img = cv2.imdecode(nparr, -1)
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_IMG_HUMANSEG_GPU_DUKA.py
    生成压测数据：
        python FEATURE_VIS_IMG_HUMANSEG_GPU_DUKA.py GEN_STRESS_DATA
    """
