...
Brief:
@Author: yexiang02
@Date:   2020-05-29 19:35:44
@Filename: FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU.py
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU demo
    """
    def prepare_request(self, data):
        ...
        req_data = {
            "appid": "FEATURE_DE_IMG_INTERSECTION_DETECTION_GPU_V1", #填写视频中台的作业名, 方便排查问题
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU.py
    生成压测数据：
        python FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        #生成压测词表
        input_data = {
            'img': './image_dir/FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU/'
        }
        gen_stress_data(input_data)
    else:
        #特征计算Demo
        input_data = {
            'img': './image_dir/FEATURE_DE_IMG_INTERSECTIONDETECTION_GPU/img.jpg'
        }
