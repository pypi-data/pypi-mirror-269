...
################################################################################
"""
Brief:FEATURE_VIS_IMG_INPAINTING_GPU Demo, 以及压测数据生成的DEMO
Author: liufanglong(liufanglong@baidu.com)
Date: 2023-12-14
Filename: FEATURE_VIS_IMG_INPAINTING_GPU.py 
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_IMG_INPAINTING_GPU demo  
    """
    ...
    def prepare_request(self, data):
        ...
        new_data = {
            ...
            'locations': [{'top': 10, 'left': 520, 'right': 620, 'bottom': 80}, 
                          {'top': 290, 'left': 130, 'right': 500, 'bottom': 330}],
        }
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_VIS_IMG_INPAINTING_GPU',
    ...
    # print(eval(res_data).keys())
    # 打印输出
    # feature_demo.parse_result(res_data)

    # 打印base64解码后的输出result
    res_json = json.loads(json.loads(res_data)["feature_result"]["value"])
    # print res_json
    if res_json['err_no'] == 0:
        res_json = json.loads(base64.b64decode(res_json['result']))
        print(res_json.keys())
        img = base64.b64decode(res_json["output_image"])
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_IMG_INPAINTING_GPU.py 
    生成压测数据：
        python FEATURE_VIS_IMG_INPAINTING_GPU.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测词表
        gen_stress_data('./image_dir/FEATURE_VIS_IMG_INPAINTING_GPU')  # ./image_dir/ 是本地的图片数据
    else:
        # 特征计算Demo
