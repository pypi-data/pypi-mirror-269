...
################################################################################
"""
Brief:FEATURE_VIS_VIDEO_DELOGO_GPU_ASYNC_T4 Demo, 以及压测数据生成的DEMO
Author: liufanglong (liufanglong@baidu.com)
Date: 2022-11-30
Filename: FEATURE_VIS_VIDEO_DELOGO_GPU_ASYNC_T4.py
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_VIDEO_DELOGO_GPU_ASYNC_T4 demo
    """
    def prepare_request(self, data):
        ...
        输出：
            返回算子的输入数据
        """
        data_tmp = './text_dir/mcm24.json'
        with open(data_tmp, 'r') as f:
            data_json = json.load(f)

        data_new = {
            "args":["videoCover"],
            "stdin": json.dumps(data_json),
            "mode": "async",
            "enableStdout": True,
            "enableStderr": True,
        }
        ...
        json_str = json.dumps({
                        ...
                        'data': base64.b64encode(json.dumps(data_new))
                    })
        # json_str = data_new
        return json_str
...
def feature_calculate(input_data):
    ...
    xvision_data = {
            ...
            'feature_name': 'FEATURE_ACG_VIDEO_VIDEOCOVER_CPU',           # 算子名
    ...
    res_data = featureDemo.request_feat_new(params, json.dumps(xvision_data), url, http_headers)
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_VIDEO_DELOGO_GPU_ASYNC_T4.py
    """
    #特征计算Demo
    test_video_url = "./text_dir/mcm24.json"
