...
class FeatureReq(XvisionDemo):
    """
    FEATURE_NETDISK_IMG_VECTOR_CLASSIFY_GPU_V1 demo  
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_NETDISK_IMG_VECTOR_CLASSIFY_GPU_V1',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_NETDISK_IMG_VECTOR_CLASSIFY_GPU_V1.py 
    生成压测数据：
        python FEATURE_NETDISK_IMG_VECTOR_CLASSIFY_GPU_V1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测词表
        gen_stress_data('image dir')
    else:
        # 特征计算Demo
