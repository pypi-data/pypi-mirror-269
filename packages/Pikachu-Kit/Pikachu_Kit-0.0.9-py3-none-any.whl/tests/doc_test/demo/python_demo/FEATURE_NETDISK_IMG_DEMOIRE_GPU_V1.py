...
from proto.FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1 import general_pb2
from proto.FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1.protobuf_to_dict import protobuf_to_dict
import json
import base64
import random
import requests
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1 demo  
    """
...
def feature_calculate(input_data, out_file):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1.py 
    生成压测数据：
        python FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测词表
        # gen_stress_data('./image_dir/FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1/')
        gen_stress_data('/mnt/bae/webroot/moire_testB_dataset')
    else:
        # 特征计算Demo
        feature_calculate('./image_dir/FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1/img.jpg', 
                          './image_dir/FEATURE_NETDISK_IMG_DEMOIRE_GPU_V1/img_res.jpg')
