...
def prepare_req(video_path, info):
    ...
    xvision_data = {
        ...
        'data': base64.b64encode(json.dumps(demo))
    }
    url = 'http://' + info['server'] + '/xvision/xvision_sync'
    url += '?business_name=%s&feature_name=%s' % (
        xvision_data['business_name'], xvision_data['feature_name'])

    req = urllib2.Request(url, data=json.dumps(xvision_data))
