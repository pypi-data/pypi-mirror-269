...
Date: 2020-05-28
Filename: FEATURE_VIS_VIDEO_ENHANCE_GPU_ASYNC_V1.py
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_VIDEO_ENHANCE_GPU_ASYNC_V1 demo
    """
    def prepare_request(self, data):
        ...
        s1   = 1000
        e1   = 3000
        m1   = base64.b64encode(open(mfn1, 'rb').read())
        ...
        s2   = 5000
        e2   = 8000
        m2   = base64.b64encode(open(mfn2, 'rb').read())
        ...
        json_str  = '{"video_url": "%s",
                    "time_mask": [
                        {"start": "%s", "end": "%s", "maskimg": "%s", "boxes": [[%d,%d,%d,%d],[%d,%d,%d,%d]]},
                        {"start": "%s", "end": "%s", "maskimg": "%s", "boxes": [[%d,%d,%d,%d]]}
                    ]}'
        json_str = json_str % (data["video_url"], s1, e1, m1, t11, l11, h11, w11,
                t12, l12, h12, w12, s2, e2, m2, t21, l21, h21, w21)
...
def feature_calculate(input_data):
    ...
    xvision_data = {
            ...
            'feature_name': 'FEATURE_VIS_VIDEO_ENHANCE_GPU_ASYNC_V1',           # 算子名
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_VIDEO_ENHANCE_GPU_ASYNC_V1.py
    生成压测数据：
        python FEATURE_VIS_VIDEO_ENHANCE_GPU_ASYNC_V1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        #生成压测词表
        gen_stress_data('./video_url_dir/url_file') #./image_dir/ 是本地的图片数据,生成压测数据需要把产品线的数据放到image_dir下边,一张图片一个文件
    else:
        #特征计算Demo
