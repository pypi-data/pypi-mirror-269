...
class FeatureReq(XvisionDemo):
    """
        FEATURE_DOCUMENT_COMPARE_V1 demo
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_DOCUMENT_COMPARE_V1',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_DOCUMENT_COMPARE_V1.py
    生成压测数据：
        python FEATURE_DOCUMENT_COMPARE_V1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测数据
        gen_stress_data('./text_dir/document_compare_testcase.txt')
    else:
        # 一行数据Demo
        source_id_data = json.load(open(f"./text_dir/document_compare_testcase.json", 'r', encoding='utf-8'))
        feature_calculate(source_id_data)