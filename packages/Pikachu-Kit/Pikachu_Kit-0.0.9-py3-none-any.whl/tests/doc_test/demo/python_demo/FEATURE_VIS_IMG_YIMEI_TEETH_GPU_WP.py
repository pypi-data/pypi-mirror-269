...
################################################################################
"""
Brief:FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP Demo, 以及压测数据生成的DEMO
Author: caijinhai(caijinhai@baidu.com)
Date: 2022-01-25
Filename: FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP.py 
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP demo
    ai修图-医美牙齿-bdes加密
    """
    def prepare_request(self, images):
        ...
        input = {
            ...
            "imagesnum": len(images),  # 必须，int，图片个数，必须和images数量匹配
            "images": images,
            "targetfacenum": 1,  # 可选，int，每张图期望保留的人脸数，不传默认为1
            "face_conf_thr": 0.8,  # 可选，float，指定人脸检测阈值，小于该值不返回，不传以配置文件设置为准
        }

        # 参数外层
        params = {
            "provider": "teethalignment",
...
def feature_calculate(input_data):
    ...
    job_name = ""  # 申请的作业名
    token = ""  # 作业的token
    feature_name = 'FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP'
    ...
    #   'logid': 8962231798, 
    #   'feature_result': {
    #       'feature': 'FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP', 
    #       'value': '{
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP.py 
    生成压测数据：
        python FEATURE_VIS_IMG_YIMEI_TEETH_GPU_WP.py GEN_STRESS_DATA
    """
