...
# coding=utf-8
"""
Author: chenziping01@baidu.com
message: FEATURE_KG_TEXT_MAINDEMAND_CPU_V1 Demo, 以及压测数据生成的DEMO
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_KG_TEXT_MAINDEMAND_CPU_V1 demo
    """
    ...
    def prepare_request(self, data):
        ...
        Args:
            data (dict): key is [query, mention]
...
def feature_calculate(input_data):
    ...
    Args:
        input_data (dict): key is [query, mention]
    输出：
        实体链指结果
    """
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_KG_TEXT_MAINDEMAND_CPU_V1',
...
def gen_stress_data(input_file):
    ...
    功能：生成压测数据
    输入：
        input_file:待链指的文本，json 形式的 dict, key 是 query 和 mention，每一行是一个json字符串
    输出：
        压测数据
    """
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_KG_TEXT_MAINDEMAND_CPU_V1.py
    生成压测数据：
        python FEATURE_KG_TEXT_MAINDEMAND_CPU_V1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测词表
        gen_stress_data('./text_dir/kg_text_main_demand_data.txt')
    else:
        # 特征计算Demo
        input = {
            "query": "周杰伦本草纲目的作者",
