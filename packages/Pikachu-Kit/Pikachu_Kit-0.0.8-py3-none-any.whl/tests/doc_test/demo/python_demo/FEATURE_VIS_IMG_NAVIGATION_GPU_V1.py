...
def feature_calculate(input_data):
	...
	headers = {
		...
		'auth_key': "e07dcd33-a991-5a47-839f-4336a2183249",
		'business_name': "map_navi_road_explanation", #job_name,
		'feature_name': 'FEATURE_VIS_IMG_NAVIGATION_GPU_V1',
	...
	img = cv2.imread(input_data['img'])
	res_json = json.loads(res_data)['feature_result']['value']
	res_json = json.loads(res_json)
	res_json = base64.b64decode(res_json['result'])
	if res_json:
		# print('======', res_json)
		res_json = json.loads(res_json)
		for bb in res_json['instances']:
			cats_id = bb['category_id']
			score = round(bb['confidence'], 2)
			...
			cv2.putText(img, str(cats_id) + '-' + str(score) + '-' + str(on_road), 
				(x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)

		# cv2.imwrite('show/' + input_data['img'].split('/')[-1], img)
