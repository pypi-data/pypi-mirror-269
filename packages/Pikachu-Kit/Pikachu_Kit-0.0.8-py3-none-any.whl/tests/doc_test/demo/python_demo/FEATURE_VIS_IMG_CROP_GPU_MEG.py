...
################################################################################
"""
Brief:FEATURE_VIS_IMG_CROP_GPU_MEG Demo, 以及压测数据生成的DEMO
Author: liufanglong(liufanglong@baidu.com)
Date: 2021-05-26
Filename: FEATURE_VIS_IMG_CROP_GPU_MEG.py 
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_IMG_CLASSIFICATION4W_GPU_V1 demo  
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_VIS_IMG_CROP_GPU_MEG',
    ...
    res_json = json.loads(res_json['feature_result']['value'])
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_IMG_CROP_GPU_MEG.py 
    生成压测数据：
        python FEATURE_VIS_IMG_CROP_GPU_MEG.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测词表
        gen_stress_data('./image_dir/FEATURE_VIS_IMG_CROP_GPU_MEG')  # ./image_dir/ 是本地的图片数据
    else:
        # 特征计算Demo
        feature_calculate('./image_dir/FEATURE_VIS_IMG_CROP_GPU_MEG/test_img.jpg')  # ./image_dir/img_file 本地图片
