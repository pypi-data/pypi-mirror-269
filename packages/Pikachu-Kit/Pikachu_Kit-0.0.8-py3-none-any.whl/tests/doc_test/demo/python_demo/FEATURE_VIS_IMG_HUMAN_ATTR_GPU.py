...
################################################################################
"""
Brief:FEATURE_VIS_IMG_HUMAN_ATTR_GPU Demo, 以及压测数据生成的DEMO
Author: xieshuai(xieshuai@baidu.com)
Date: 2024-01-25
Filename: FEATURE_VIS_IMG_HUMAN_ATTR_GPU.py
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_IMG_HUMAN_ATTR_GPU demo
    """
...
def feature_calculate(input_data):
    ...
    # 生成算子输入
    calc_type =  1 + 2 + 4 + 8 + 16 + 32 + 64 + 8192 + 1073741824 + 16384
    data = {
        'logid': 'test_666',
        'imagesnum': 1, # 图片个数，必须和images数量匹配
        'targetfacenum': 1, # 每张图期望保留的人脸数，不传默认为1
        'face_option': 1, # 只走人脸检测 1 是走人脸属性模型
        'human_option': 0, # 0是只走皮肤检测 1 是走皮肤属性模型
        'pic_option': 0, # 0是不走全图皮肤分割 1是走全图皮肤分割
        'scene_type': 100,
        # images必须，请求的图片列表
        'images': [
            {
                "imageid": input_data["image"],	 # 必须，string，图片id
                "image": base64.b64encode(Util.read_file(input_data["image"]))	 # 必须，string，图片的base64字符串
            }
        ]
    }
    input_data_1 = {
        'provider': 'get_humanattr',
        'input': json.dumps(data)
    }

    job_name = ""  # 申请的作业名
    token = ""  # 作业的token
    feature_name = "FEATURE_VIS_IMG_HUMAN_ATTR_GPU" # 算子名称
    headers = {
        'Content-Type': 'application/json; charset=UTF-8',
        # 'resource_key': 'test.jpg',
        # 'auth_key': token,
        # 'business_name': job_name,
        # 'feature_name': 'FEATURE_VIS_IMG_HUMAN_ATTR_GPU',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_IMG_HUMAN_ATTR_GPU.py
    生成压测数据：
        python FEATURE_VIS_IMG_HUMAN_ATTR_GPU.py GEN_STRESS_DATA
    """
