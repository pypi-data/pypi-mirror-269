...
################################################################################
"""
Brief:FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU Demo, 以及压测数据生成的DEMO
Author: caijinhai(caijinhai@baidu.com)
Date: 2021-09-23
Filename: FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU.py 
"""
...
class FeatureReq(XvisionDemo):
    """
    FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU demo  
    """
...
def feature_calculate(input_data):
    ...
    功能：特征计算
    输入：
        input_data: {'req_image_id': '', 'req_video_id': '', 'track_url': ''}
    输出：
        图片特征
    """
    ...
    job_name = ""  # 申请的作业名
    token = ""  # 作业的token
    feature_name = 'FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU'
    ...
    data = json.dumps({
        ...
        # callback 信息，可选参数
        'callback': json.dumps({
            "path": "/face-api/videomerge/task/callback",
...
def gen_stress_data(image_input_file):
    ...
    for image_url in image_urls:
        data = {
            ...
            "req_video_id": "https://facefusion-2021-peoplesdaily.cdn.bcebos.com/\
            template/9466d211-602c-4de5-95b4-f0844d80668d.mp4",
            "track_url": "https://facefusion-2021-peoplesdaily.cdn.bcebos.com/track/\
            9466d211-602c-4de5-95b4-f0844d80668d_v1_extract.tar.gz",
        }
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU.py 
    生成压测数据：
        python FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU.py GEN_STRESS_DATA
    """
    # 执行流程：
    # 1. 下载图片
    ...
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测数据
        # image.txt: 图片url列表文件
        gen_stress_data('./video_url_dir/FEATURE_VIS_VIDEOIMG_VIDEOMERGE_GPU/image.txt')
    else:
        # 合成视频
        data = {
