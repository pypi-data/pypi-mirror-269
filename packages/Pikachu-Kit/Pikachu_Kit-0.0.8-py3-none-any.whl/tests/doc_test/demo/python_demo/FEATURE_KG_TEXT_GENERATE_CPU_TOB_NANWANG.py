...
Date: 2021-08-31 14:25:42
Description: FEATURE_KG_TEXT_GENERATE_CPU_TOB_NANWANG demo, 压测数据生成的demo
FilePath: /demo/python_demo/FEATURE_KG_TEXT_GENERATE_CPU_TOB_NANWANG.py
"""
...
class FeatureReq(XvisionDemo):
    """
        FEATURE_KG_TEXT_GENERATE_CPU_TOB_NANWANG demo
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_KG_TEXT_GENERATE_CPU_TOB_NANWANG',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_KG_TEXT_GENERATE_CPU_TOB_NANWANG.py
    生成压测数据：
        python FEATURE_KG_TEXT_GENERATE_CPU_TOB_NANWANG.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测数据
        gen_stress_data('./text_dir/xvision_nanwang_weeklyreport_testcase.txt')
    else:
        # 周报生成Demo
        with open('./text_dir/xvision_nanwang_weeklyreport_testcase.txt', 'r') as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
