...
#
"""
File: FEATURE_KG_GPU_EMO_PARAPHRASE1.py
Author: wuyuwei(wuyuwei@baidu.com)
Date: 2022/3/29 2:34 下午
"""
...
class FeatureReq(XvisionDemo):
    """
        FEATURE_KG_GPU_EMO_PARAPHRASE1 demo
    """
...
def feature_calculate(input_data):
    ...
    headers = {
        ...
        'feature_name': 'FEATURE_KG_GPU_EMO_PARAPHRASE1',
...
if __name__ == '__main__':
    ...
    main
    特征计算执行：
        python FEATURE_KG_GPU_EMO_PARAPHRASE1.py
    生成压测数据：
        python FEATURE_KG_GPU_EMO_PARAPHRASE1.py GEN_STRESS_DATA
    """
    op_type = sys.argv[1] if len(sys.argv) == 2 else 'FEATURE_CALCULATE'
    if op_type == 'GEN_STRESS_DATA':
        # 生成压测数据
        gen_stress_data('./text_dir/xvision_EmoParaphrase_testcase.txt')
    else:
        # 智能改写Demo
        with open('./text_dir/xvision_EmoParaphrase_testcase.txt', 'r') as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
