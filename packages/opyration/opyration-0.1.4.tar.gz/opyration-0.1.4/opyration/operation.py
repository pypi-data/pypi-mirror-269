...
class Operation(object):
    ...
    def _parameterize(self, **pairs):
        if not self.__vals: self.__vals = []
        keys, vals = [], []
    ...
    def cte(self, name):
        self.__cte = True
        self.__sql = f'WITH {name} AS (@@@) '
        return self

    def wrap(self, op: 'Operation'):
        self.__sql = self.__sql.replace('@@@', op.sql)
        if self.__vals: self.__vals.append(op.parameters)
        else: self.__vals = op.parameters
        return self
