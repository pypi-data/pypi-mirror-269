...
BASE_DOCKER_REPO = "slalomdata/tapdance"
...
def _get_plugins_list(
    ...
    """
    Load the contents of the singer_index.yml file or a custom file to get plugin list. 
    Return a list of tuples that contains the plugin name, source, and alias.
    """
        ...
        for target_name, target_source, target_alias in plugins:
            ...
            if tap_alias.startswith("tap-") and target_alias.startswith("target-"):
                ...
                if has_custom_tap and has_custom_target:
                    ...
                    #create a docker image for the tap and target combination
                    created_images.append(
...
def _build_plugin_image(
    ...
    """
    Build an image for a plugin.

    Returns the name of the image that is created. 
    The image will be created and pushed to dockerhub.
    """
...
def _build_composite_image(
    ...
    *,
    ...
    """
    Create a docker image for a tap and target combination
...
def build_all_images(
    ...
    """
    #build all the standalone images
    built_images = _build_all_standalone(push=push, pre=pre, ignore_cache=ignore_cache)
    #build all combinations of taps and targets form the singer_index.yml
