...
def pandas_convert_odfv_only(tree: NodeRef, odfv_input_df: pandas.DataFrame) -> PandasExecNode:
    assert tree_contains(tree, MultiOdfvPipelineNode)
    return _convert_to_pandas_nodes(tree, odfv_input_df)


def _convert_to_pandas_nodes(tree: NodeRef, odfv_input_df: pandas.DataFrame) -> PandasExecNode:
    ...
    if isinstance(logical_tree_node, StagedTableScanNode):
        return PandasDataNode(
            input_reader=pyarrow.Table.from_pandas(odfv_input_df).to_reader(),
            input_node=None,
