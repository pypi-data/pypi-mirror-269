...
class MemoryInterface(object):
    ...
    def write_byte(self, cycle, address):
        pass
...
class BasicMemory(RAM):
    ...
    def read_word(self, cycle, address):
        return self.read_byte(cycle, address) + (
            self.read_byte(cycle + 1, address + 1) << 8
        )

    def read_word_bug(self, cycle, address):
        if address % 0x100 == 0xFF:
            return self.read_byte(cycle, address) + (
                self.read_byte(cycle + 1, address & 0xFF00) << 8
            )
        else:
...
class CPU(object):
    ...
    def read_byte(self, address):
        return self.bus.read_byte(self.cycles, address)

    def read_word(self, address):
        return self.bus.read_word(self.cycles, address)

    def read_word_bug(self, address):
        return self.bus.read_word_bug(self.cycles, address)
    ...
    def write_byte(self, address, value):
        self.bus.write_byte(self.cycles, address, value)
    ...
    def ASL(self, operand_address=None):
        if operand_address is None:
            ...
            self.write_byte(
                operand_address,
                self.update_nzc(self.read_byte(operand_address) << 1),
            )
    ...
    def LSR(self, operand_address=None):
        if operand_address is None:
            ...
            self.write_byte(
                operand_address,
                self.update_nz(self.read_byte(operand_address) >> 1),
            )
    ...
    def AND(self, operand_address):
        self.accumulator = self.update_nz(
            self.accumulator & self.read_byte(operand_address)
        )
