...
def test_current_invoke_id() -> None:
    client = ADSClient(local_ams_net_id="192.168.0.100.1.1")
...
def test_send_ams_packet_timeout() -> None:
    client = ADSClient(local_ams_net_id="192.168.0.100.1.1", timeout_s=0.1)
    ...
    with mock.patch("socket.socket") as mock_socket_class:
        ...
        client.open(target_ip=target_ip, target_ams_net_id="192.168.0.200.1.1")
        ...
        mock_socket.sendall.assert_called_once_with(
            bytes.fromhex(
                "00 00"
                + "24 00 00 00"
                + "c0 a8 00 c8 01 01 53 03 c0 a8 00 64 01 01 40 1f"
...
def test_read_socket_background() -> None:
    ...
    ams_header = bytes.fromhex(
        "c0 a8 00 c8 01 01 53 03 c0 a8 00 64 01 01 40 1f 02 00 04 00 04 00 00 00 00 00 00 00 01 00 00 00"
    )
...
def test_handle_ams_raw_packet() -> None:
    ...
    header = AMSHeader(
        target_net_id="192.168.0.100.1.1",
        target_port=8000,
        source_net_id="192.168.0.200.1.1",
        source_port=851,
