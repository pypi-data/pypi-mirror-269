...
def test_ams_header() -> None:
    raw_data = bytes.fromhex("c0a800b101015303c0a800140202307509000400370000000000000001000000")
    header = AMSHeader.from_bytes(raw_data)

    assert header.target_net_id == "192.168.0.177.1.1"
    assert header.target_port == 851
    assert header.source_net_id == "192.168.0.20.2.2"
