...
def dict_to_xml(input_dict):
    # Determine the root tag based on the number of keys in the dictionary
    if len(input_dict) == 1:
    ...
    def element_to_dict(element):
        ...
        for child in element:
            ...
            if child.tag in dict_data:
                # If tag is already present in the dictionary, append to a list
                if type(dict_data[child.tag]) is list:
                    dict_data[child.tag].append(child_dict[child.tag])
                else:
                    dict_data[child.tag] = [dict_data[child.tag], child_dict[child.tag]]
            else:
                # Insert new tag
                dict_data.update(child_dict)
        return {element.tag: dict_data}

    # Parse the XML from string and convert to dictionary
    root = ET.fromstring(xml_data)
    return element_to_dict(root)[root.tag]
