...
def deserialize_data(
    ...
    try:
        ...
        if response.content_type == "application/xml" or response.content_type == "text/xml":
...
def dict_to_xml(input_dict):
    ...
    if len(unique_key_lengths) == 1 and len(input_dict) == 1:
        root_tag = next(iter(input_dict))  # Use the first key as the root tag if all keys have the same length and only one key
        if isinstance(input_dict[root_tag], dict):  # Ensure that the value is also a dictionary
            input_dict = input_dict[root_tag]
        else:
            raise TypeError("Value under single key must be a dictionary for use as XML structure.")
    else:
    ...
    def build_element(parent, dict_obj):
        if isinstance(dict_obj, dict):
            ...
            for item in dict_obj:
                item_element = ET.SubElement(parent, "item")  # Create generic 'item' elements for list items
