...
class BaseSupportTicketMessageView(CustomModelViewSet):
    ...
    def set_viewed(self, request, *args, **kwargs):
        """
        Пометить объект как просмотренный пользователем
        """
        instance = self.get_object()
        instance.set_viewed()
        return Response(data={"message": "viewed_message"})

    @action(detail=False, methods=["POST"], url_path="set_viewed")
    def bulk_set_viewed(self, request, *args, **kwargs):
        """
        Массовая пометка объектов как просмотренных пользователем
        """
        self.get_queryset_by_ids(request=request).update(is_viewed=True)
        return Response(data={"message": "viewed_message_plural"})
