from finalsa.common.models import SqsMessage, to_message_attributes
...
class SnsClientImpl(SnsClient):
    ...
    def list_subscriptions(self, topic: str) -> List:
        try:
            ...
            return self.get_client().list_subscriptions_by_topic(TopicArn=topic.arn)['Subscriptions']
        except Exception as ex:
    ...
    def publish(self, topic_name: str, payload: Union[Dict, str], att_dict: Optional[Dict] = {}) -> Dict:
        ...
        response = topic.publish(
            Message=payload, MessageAttributes=to_message_attributes(att_dict))
        message_id = response['MessageId']
        self.logger.info(
            "Published message with attributes %s to topic %s.", att_dict,
            topic.arn)
        return {
            'id': message_id,
            'topic': topic.arn,
