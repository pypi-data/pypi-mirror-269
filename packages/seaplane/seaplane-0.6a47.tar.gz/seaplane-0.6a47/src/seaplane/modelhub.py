...
class Modelhub:
    ...
    def get_response(self, msg: Generator[Any, None, None]) -> Generator[Any, None, None]:
        return self.simplify_output(self.substation.get_response(msg))
...
<|im_start|>assistant"""
    ...
    def simplify_input(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if input.get("prompt"):
            ...
            if not input.get("model"):
                raise ValueError("Model name is required")
            elif not input.get("prompt"):
                raise ValueError("Prompt is required")
            else:
                # wrap in prompt template if requested default is yes
                if input.get("use_prompt_template", True) and model in [
                    "zephyr-7b-beta",
            ...
            if model == "starling-lm-7b-alpha":
                # set the max_tokens if provided
                if input.get("max_new_tokens"):
                    output["max_tokens"] = input.get("max_new_tokens")

            # FALCON MODEL FAMILY
            if model == "falcon-40b-instruct":
                # set the max_tokens if provided
                if input.get("max_new_tokens"):
    ...
    def simplify_output(self, msg: Any) -> Generator[Any, None, None]:
