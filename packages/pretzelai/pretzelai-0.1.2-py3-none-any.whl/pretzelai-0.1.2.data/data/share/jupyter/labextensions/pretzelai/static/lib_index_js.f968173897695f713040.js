...
const extension = {
    ...
    activate: async (app, palette, notebookTracker, settingRegistry) => {
        ...
        let embeddings;
        async function createEmbeddings(embeddingsJSON, cells, embeddingsPath) {
            let newEmbeddings = false;
            embeddings = embeddingsJSON;
            const promises = cells.map(cell => {
                return (async () => {
                    const index = embeddings.findIndex(e => e.id === cell.id);
                    if (index !== -1) {
                        const hash = await (0,_utils__WEBPACK_IMPORTED_MODULE_5__.calculateHash)(cell.source);
                        if (hash !== embeddings[index].hash) {
                            try {
                                console.log('Hash changed: creating embedding for cell ', cell.id);
                                newEmbeddings = true;
                        ...
                        console.log('New cell: creating embedding for cell ', cell.id);
                        newEmbeddings = true;
                        try {
                            const response = await openai.embeddings.create({
                                model: EMBEDDING_MODEL,
        ...
        function getEmbeddings() {
            ...
            if ((notebook === null || notebook === void 0 ? void 0 : notebook.model) && openai) {
                ...
                app.serviceManager.contents
                    ...
                    .catch(async (error) => {
                    createEmbeddings([], notebook.model.sharedModel.cells, embeddingsPath).then(() => {
                        app.serviceManager.contents.save(embeddingsPath, {
                            type: 'file',
                            format: 'text',
                            content: JSON.stringify(embeddings)
                            ...
                            if (isLocalhost) {
                                try {
                                    const complete = async () => {
                                        ...
                                        const similarities = embeddings.map(embedding => (0,_utils__WEBPACK_IMPORTED_MODULE_5__.cosineSimilarity)(embedding.embedding, userInputEmbedding));
                                        const topThreeSimilarities = similarities
                                            .map((value, index) => ({ value, index }))
                                            .sort((a, b) => b.value - a.value)
                                            .slice(0, NUMBER_OF_SIMILAR_CELLS)
...
//# sourceMappingURL=lib_index_js.f968173897695f713040.js.map