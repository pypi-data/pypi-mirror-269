...
impl StoreServicesCoreADIProxy {
    ...
    fn get_guid_fields(&mut self, flag: u32) -> PyResult<GuidData> {
        let mut guid = vec![0u8; 20]; // Ensure there is enough space
        let guid_ptr: *mut u8 = guid.as_mut_ptr();

        unsafe {
            let result = (self.fairplay_get_guid_fields)(guid_ptr, flag);
            if result == 0 {
                // Convert the raw buffer into a Rust Vec up to the first null byte
                let len = (0..20)
                    .take_while(|&i| *guid_ptr.offset(i as isize) != 0)
                    .count();
                let guid_data = Vec::from_raw_parts(guid_ptr, len, 20);
