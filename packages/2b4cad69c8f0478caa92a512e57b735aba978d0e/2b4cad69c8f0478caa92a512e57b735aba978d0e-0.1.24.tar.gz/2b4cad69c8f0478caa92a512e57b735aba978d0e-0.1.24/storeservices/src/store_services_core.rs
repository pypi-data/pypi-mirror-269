...
pub struct StoreServicesCoreADIProxy {
    ...
    fairplay_get_guid_fields: sysv64_type!(fn(out_guid: *mut *const u8, flag: u32) -> i32),
...
impl StoreServicesCoreADIProxy {
    ...
    fn get_guid_fields(&mut self, flag: u32) -> PyResult<GuidData> {
        unsafe {
            ...
            let result = (self.fairplay_get_guid_fields)(&mut guid_ptr, flag);

            if result == 0 {
                // Return the guid pointer + 1
                let mut guid_data = vec![0; 18];

                guid_data.copy_from_slice(std::slice::from_raw_parts(guid_ptr.wrapping_add(1), 18));
