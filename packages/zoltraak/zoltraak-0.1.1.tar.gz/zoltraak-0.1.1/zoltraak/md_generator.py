...
def print_generation_result(target_file_path, open_file=True):
    ...
    Args:
        ...
        open_file (bool): ファイルを開くかどうかのフラグ（デフォルトはTrue）
    """
    print(f"要件定義書を生成しました: {target_file_path}")  # 要件定義書の生成完了メッセージを表示
    print(f"以下のコマンドをコピーして、ターミナルに貼り付けて実行してください。")  # 実行方法の説明を表示
    print(f"zoltraak{target_file_path}")  # - 実行コマンドを表示
    pyperclip.copy(f"zoltraak{target_file_path}")  # - 実行コマンドをクリップボードにコピー
    print("コマンドをクリップボードにコピーしました。ターミナルに貼り付けて実行できます。")  # コピー完了メッセージを表示
