...
class Database(auth.Authentication):
    ...
    def get_vtile_qc(self, qrcode, grade=None):
        ...
        def check_grade(qrcode, grade):
            """
            Given a QR code and a GradeA/B returns True/False
            """
            import math
            import json
            ...
            for variable in variables:
                ...
                if v < limits[f"{variable}_min"] or v > limits[f"{variable}_max"] or math.isnan(v):
                    # variable already out of the limits or NaN, so so could not have the asked grade
                    return False
        ...
        bias_voltage=69 # QA/QC are evaluated for 69V
        ...
        variables = ['snr', 'amplitude_1pe', 'charge_1pe', 'dark_noise', 'cda', 'cross_talk', 'rms', 'breakdown', 'apa']

        if grade is not None:
            # check for the given specific grade to return True/False
            return(check_grade(qrcode,grade))
        ...
        if check_grade(qrcode,"a"):
            return "grade_a"
        elif check_grade(qrcode,"b"):
