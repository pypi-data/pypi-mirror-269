...
@patch_to(DomoJupyterWorkspace, cls_method=True)
async def get_current_workspace(cls: DomoJupyterWorkspace, auth: dmda.DomoAuth):
    try:
    ...
    return await cls.get_by_id(workspace_id=workspace_id, auth=auth)


@patch_to(DomoJupyterWorkspace)
async def get_accounts(self: DomoJupyterWorkspace):
    ...
    async def _get_accounts(account_id, auth, props):
        domo_account = await dmac.DomoAccount.get_by_id(
            account_id=account_id, auth=auth
        )
    ...
    self.domo_accounts_config = await ce.gather_with_concurrency(
        *[
            _get_accounts(
                account_id=account_obj["account_id"], auth=self.auth, props=account_obj
            )
