...
@patch_to(dmdj.DomoJupyterWorkspace, cls_method = True)
async def get_current_workspace(cls: dmdj.DomoJupyterWorkspace, auth : dmda.DomoAuth):
    try:
        workspace_id = os.environ['DOMO_WORKSPACE_ID']
    
    except KeyError as e:
        raise DomoJupyter_InvalidWorkspace(message = "key error | workspace id not found.  This only works in Domo Jupyter Workspaces", domo_instance= auth.domo_instance)
    
    return await cls.get_by_id(workspace_id = workspace_id,
                               auth = auth)

@patch_to(dmdj.DomoJupyterWorkspace)
async def get_accounts(self : dmdj.DomoJupyterWorkspace):
    
    async def _get_accounts(account_id, auth, props):
            domo_account = await dmac.DomoAccount.get_by_id( account_id = account_id , auth = auth)
            domo_account.alias = props['alias']
            return domo_account
    
    self.domo_accounts_config = await ce.gather_with_concurrency(*[ _get_accounts( account_id = account_obj['account_id'] , auth = self.auth, props = account_obj) for account_obj in self.account_configuration], n = 5)
