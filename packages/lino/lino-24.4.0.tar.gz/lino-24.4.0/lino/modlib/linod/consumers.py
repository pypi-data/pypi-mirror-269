...
class LinodConsumer(AsyncConsumer):
    ...
    async def send_push(self, event):
        ...
        user = event['user_id']
        if user is not None:
            user = await database_sync_to_async(settings.SITE.models.users.User.objects.get)(pk=user)
        kwargs = dict(
    ...
    async def send_panel_update(self, event):
        ...
        data = dict([(a.actor_id, {"mk": None, "mt": None, "pk": msg["pk"]}) for a in ups if not is_slave(a)])
        if msg['mk'] is not None:
            MasterModel = rt.models.resolve(msg["master_model"])
            ContentType = rt.models.contenttypes.ContentType
            mt = (await database_sync_to_async(ContentType.objects.get_for_model)(MasterModel)).pk
            data.update(**dict([(a.actor_id, {"mk": msg["mk"], "mt": mt, "pk": msg["pk"]}) for a in ups
                                if is_slave(a) and match_master(ContentType, MasterModel, a.master)]))
        await self.channel_layer.group_send(BROADCAST_CHANNEL, {
            "type": "send.notification", "text": json.dumps({"type": msg["type"], "data": data})})
