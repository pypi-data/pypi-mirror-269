...
signature as `django.core.mail.EmailMessage`.
    ...
    def action_link(self, spec, obj=None, text=None, title=None, pv={}):
        ...
        from lino.core.actors import resolve_action
        ba = resolve_action(spec)
        # ar = ba.request(parent=self, action_param_values=apv)
        # url = self.renderer.js2url(self.renderer.request_handler(ar))
        url = self.renderer.get_permalink(self, ba, obj, **pv)
        return tostring(self.renderer.href_button(url, str(text), title))
