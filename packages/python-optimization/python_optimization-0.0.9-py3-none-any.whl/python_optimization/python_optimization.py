...
predicted_prices = lr.predict(X_scaled)

data['Actual Price'] = data['Price'] 
...
sns.scatterplot(x='Actual Price', y='Predicted Price', data=data, alpha=0.5)
...
try:
    spamdf = pd.read_csv("spam.csv")
except:
    spamdf = pd.read_csv("AImodels\\spam_detection\\spam.csv")
...
cv = CountVectorizer()
x_train_valid = cv.fit_transform(x_train.values)

model = MultinomialNB()
model.fit(x_train_valid, y_train)
...
if __name__ == "__main__":
    print(spam_detection(['3421452465 call here for reward']))
"""
    return "Package not Installed properly"
...
excel = """Clean columns - Manager names (TRIM[col]), Quantity (ROUNDUP[col, 0])
Remove Duplicates - Data Tools
Data Analysis - File -> Options -> Addins -> Analysis Toolpak -> GO -> Analysis Toolpak
Data Analysis - Descriptive Stats (Price, Quantity)
Visualize - Select Price col -> Charts -> Wisker plot ->Select Data -> Horizontal -> Select Managers Col
Create New Col - Revenue = Quan * Price
Pivot Table"""

etl = """1. Removed unnessary columns from the products table (we kept productid, productname, qtyperunit, unitinstock)
2. Changed the data type of unitsinstock from decimal to whole nos.
3. Expanded the order details table.
4. Calculated the line total for each order details row. 
5. Renamed the lineTotal column to Stock Value and shifted its position.
6. Combined the products and total sales queries"""
