...
class PassworkAPI:
    ...
    def get_full_password_info(self, password_id: str):
        password_item = self.get_password(password_id=password_id)
        vault_id = password_item.get("vaultId")
        vault_item = self.get_vault(vault_id=vault_id)
        vault_password = self.get_vault_password(vault_item=vault_item)
        password_encryption_key = self.get_password_encryption_key(
            password_item=password_item, vault_password=vault_password
        )
        password_item["custom"] = self.get_customs(
            password_item=password_item, password_encryption_key=password_encryption_key
        )
        self.get_attachments(
            password_item=password_item, password_encryption_key=password_encryption_key
        )
        password_plain_text = self.get_password_plain_text(
            password_item=password_item, password_encryption_key=password_encryption_key
        )
