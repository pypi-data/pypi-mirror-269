...
def test_user_info(business_client):
    from data_studio.server import _get_user_info, _create_user
...
@pytest.mark.parametrize('command_line, result', [
    (['data-studio', 'user', '--username', 'test@test.com', '--password', '12345678'], None),
])
@pytest.mark.django_db
...
def test_string_is_url():
    from data_studio.core.utils.common import string_is_url
...
def test_get_client_ip():
    from data_studio.core.utils.common import get_client_ip
...
def test_timestamp_now():
    from data_studio.core.utils.common import timestamp_now
...
@pytest.mark.parametrize('url, block_local_urls, raises_exc', [
    ...
    ('http://example', False, DataStudioAPIException),
    ('http://example', True, DataStudioAPIException),
    # localhost
