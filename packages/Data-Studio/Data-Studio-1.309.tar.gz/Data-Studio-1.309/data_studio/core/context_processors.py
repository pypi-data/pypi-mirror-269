...
def settings(request):
    ...
    versions['lsf'] = versions.get('data-studio-frontend', {})
    versions['lsf']['commit'] = versions['lsf'].get('commit', 'none')[0:6]

    versions['backend'] = {}
    if 'data-studio-os-backend' in versions:
        versions['backend']['commit'] = versions['data-studio-os-backend'].get('commit', 'none')[0:6]
    if 'data-studio-enterprise-backend' in versions:
        versions['backend']['commit'] = versions['data-studio-enterprise-backend'].get('commit', 'none')[0:6]
