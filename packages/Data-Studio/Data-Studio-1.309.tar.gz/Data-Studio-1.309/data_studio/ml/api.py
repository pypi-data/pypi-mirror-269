...
@method_decorator(
    ...
    Add an ML backend to a project using the Data Studio UI or by sending a POST request using the following cURL 
    command:
    ```bash
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        operation_description="""
    Update ML backend parameters using the Data Studio UI or by sending a PATCH request using the following cURL command:
    ```bash
