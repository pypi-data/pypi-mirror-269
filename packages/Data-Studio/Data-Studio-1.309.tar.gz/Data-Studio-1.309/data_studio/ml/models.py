...
def create_ml_webhook(sender, instance, created, **kwargs):
    ...
    # Debugging: Log the type and value of ml_backend.url
    logger.debug(f"ml_backend.url type: {type(ml_backend.url)}, value: {ml_backend.url}")

    # Ensure ml_backend.url is treated as a string
    if not isinstance(ml_backend.url, str):
        logger.error(f"ml_backend.url is not a string. Actual type: {type(ml_backend.url)}")
        return
    ...
    if Webhook.objects.filter(project=project, url=webhook_url).exists():
        logger.info(f'Webhook {webhook_url} already exists for project {project}. Skipping creation.')
        return

    logger.info(f'Creating ML backend webhook {webhook_url}')
    ser = WebhookSerializer(data={
        'project': project.id,
        'url': webhook_url,
        'send_payload': True,
    ...
    if ser.is_valid():
        ...
        logger.error(f"Failed to create webhook due to validation errors: {ser.errors}")
