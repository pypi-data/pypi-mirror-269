...
class Job():
    ...
    def __importa_json( self,
        ...
        dtype = self.__monta_dtype(cols_saida, None, tp_cols_entrada)
    ...
    def importa_arquivo_posicional(self, 
        ...
        thousands:      separador de milhar
        cols_entrada:   lista com o índice (zero-based) das colunas lidas que
                        serão utilizadas. Se None, utiliza todas as colunas, na
                        ordem lida.  Esse parâmetro é importante quando se quer
                        que uma única coluna do arquivo seja fonte de entrada
                        para mais de uma coluna de saída, já que a relação
                        entre cols_saida e cols_entrada tem que ser de 1 para
                        1.  Por exemplo, se a primeira coluna do arquivo for
                        fonte de entrada para a primeira e terceira colunas da
                        tabela de saída, esse parâmetro deve ser [0, 1, 0]
        tp_cols_entrada:tipos das colunas de entrada no arquivo, no formato de
                        um array de objetos TpColBD, na ordem das colunas do
                        arquivo. Se não informado, os tipos são obtidos a
                        partir do parâmetro cols_saida
        estrategia:     estratégia de gravação que será utilizada (enum
                        TpEstrategia)
        cols_chave_update:  lista com os nomes das colunas que serão chave de
                            um eventual update na tabela. Esse parâmetro só faz
        ...
        if cols_entrada is not None:
            arq_posicional.formata_colunas(cols_entrada)
