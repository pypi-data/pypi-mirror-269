...
class start_astec_cleaner(Thread):
    ...
    def get_images_at_t(self, time, folder):
        ...
        :param time: time point for the images
        :type time: string
    ...
    def clean_at_t(self, time, pyom):
        ...
        for i  in range(0,len(self.cleaner_folder)): # can have multiple working folder (for example multi channel fusion)
    ...
    def run(self):
        ...
        for current_time in range(self.begin, self.end + 1): # Clean all the time points one by one as soon as it's done (and wait for t+2 for segmentation)
            if current_time < self.end:
                ...
                for dir in self.cleaner_folder:
                    while not self.image_t_exists(self.end, dir):
                        time.sleep(10)
                    self.clean_at_t(current_time, pyom)
        if self.astec_command == "astec_fusion" and len(self.cleaner_folder) > 1: # Fusion has specific processing (no need to wait)
...
class start_astec_command(Thread):
    ...
    def run(self):
        ...
        if self.use_omero_input: # if we use omero for input data
            folder_target = compute_input_folder(self.astec_command, self.params_dict) # compute the corresponding folder
            if folder_target is None:
                print("Unable to comput input files folders , exiting")
                return
            os.makedirs(folder_target)
            pyom = omerolib.connect(file_path=self.omero_config_file)
            pyom.download_omero_set(self.omero_project_name, self.omero_project_name, folder_target) # download the input data
        ...
        for dict_key in self.params_dict:
            if not dict_key == "mars_path" and not dict_key == "begin" and not dict_key == "end" and not dict_key == "raw_delay" and not dict_key == "delta": # manual exclude params that crash if string TODO: work to make this automatic
                ...
                if not isinstance(self.params_dict[dict_key], bool) and isinstance(self.params_dict[dict_key], str) and not isinstance(self.params_dict[dict_key],list): # treat string params
...
class Manager:
    ...
    def generate_lineage(self,parameters):
        ...
        if not "begin" in parameters:
            print("begin time point (key : begin) is needed to generate lineage , exiting")
            return
        ...
        image_files = [f for f in os.listdir(fuse_path) if os.path.isfile(os.path.join(fuse_path, f)) and (".nii" in f or ".mha" in f)]
        ...
        image_files = [f for f in os.listdir(seg_path) if os.path.isfile(os.path.join(seg_path, f)) and (".nii" in f or ".mha" in f)]
        seg_image_template = ""
        ...
        command = "conda run -n astec mc-cellProperties -fusion-format "+os.path.join(fuse_path, fuse_image_template)+" -segmentation-format "+os.path.join(seg_path, seg_image_template)+ " -first "+str(begin)+ " -last "+str(end)+ " -o "+os.path.join(seg_path,str(propertyfile))+" -feature lineage -feature volume"
        print(command)
        os.system(command)
    ...
    def compute_graphs_post(self, embryo_name, begin, end,post_list):
        ...
        apply_analysis(lineages, names, folder_out, embryo_name, begin, end,
                        is_test=False, ref_lineage=None,
    ...
    def writeStepToJson(self, parameters,step,embryo_folder=".",logFolder=None):
        ...
        :param parameters: list of instance parameters
    ...
    def upload_on_omero(self, config_file, omero_project_name, omero_dataset_name, input_folder, include_logs=False,embryo_name=None,user=None):
        ...
        :param omero_project_name: Name of the project on OMERO , created if doesn't exist
        :type omero_project_name: str
        :param omero_dataset_name: Name of the dataset on OMERO, created if doesn't exist
        :type omero_dataset_name: str
    ...
    def download_whole_embryo(self,parameters):
        ...
        :param parameters: List of parameters for download
        :type parameters: list
    ...
    def compute_contours(self, parameters):
        """ Compute contours from a given fusion instance. Refer to AstecManager documentation for more details
    ...
    def name_embryo(self, parameters):
        """ Start the naming of an embryo : generate surfaces of contact if missing, than generates a initial naming by atlas. Propagate this name by ascidian atlas naming
    ...
    def start_fusion(self,parameters,run_now=True,keep_temp=False,channel_count=1,one_stack_only=False,stack_chosen=0):
        ...
        :param stack_chosen: If one stack only fusion is chosen , index of the stack to use (0 or 1) (Default value = 0)
        :type stack_chosen: int
        :returns: The parameters dictionary updated with all of the parameters automatically added
        :rtype: dict
        """
        omero_config_file = get_omero_config(parameters)
        embryo_name = parameters["embryo_name"].replace('"','').replace("'","")
    ...
    def start_mars(self,parameters):
        ...
        :param parameters: Dict of parameters for first time point segmentation
        :type parameters: dict
