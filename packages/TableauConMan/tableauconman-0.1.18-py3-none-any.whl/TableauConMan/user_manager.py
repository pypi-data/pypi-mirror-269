...
class UserManager(AssetsManager):
    ...
    def delete_user(
        self, user_name: str, admin_user_name: str, user_archive_folder: str
    ) -> bool:
        """
        ...
        admin_user = self.get_user_by_name(admin_user_name)
        ...
        with self.plan.target.connect():
            ...
            if len(workbooks) > 0:
                logger.info(
                    f"User {user_name} owns {len(workbooks)} workbooks, updating these to {admin_user_name}"
                )
                admin_user = self.get_user_by_name(admin_user_name)
