...
def execute(
    ...
    **kwargs
    ...
    Examples:
        ...
        >>> rna.process.execute("sleep 0.1", timeout=5, level=None)

        >>> rna.process.execute("sleep 0.1 && echo 'Done'", output=True, shell=True)
        ['Done']
    ...
    if not ("shell" in kwargs and kwargs["shell"]):
        prepared_command = shlex.split(system_command)
    else:
        prepared_command = system_command
    pipe = subprocess.Popen(prepared_command, **kwargs)
    timer = None
