...
class DocumentTransformationBuilder:
    ...
    def execute(self) -> Document:
        ...
        for transformation_name, params in self.transformations:
            try:
                transformer = getattr(module, transformation_name.value)(config=self.document.config, **params)
                self.document = transformer.transform(self.document)
            except Exception as e:
                raise Exception(f"Error executing transformation {transformation_name}: {e}")
        self.transformations = []
...
class Doctran:
    def __init__(self, openai_api_key: str = None, openai_model: str = "gpt-4", openai_token_limit: int = 8000, openai_api_base: str = None):
        ...
        if openai_api_base:
            self.config.openai.base_url = openai_api_base
        elif os.environ.get('OPENAI_API_BASE'):
            self.config.openai.base_url = os.environ['OPENAI_API_BASE']
