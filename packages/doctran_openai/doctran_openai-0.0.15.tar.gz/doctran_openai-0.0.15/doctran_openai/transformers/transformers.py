...
class OpenAIDocumentTransformer(DocumentTransformer):
    ...
    def executeOpenAICall(self, document: Document) -> Document:
        try:
            ...
            # Check if completion.choices and message.function_call exists
            if not completion.choices:
                raise Exception("OpenAI completion choices are empty.")
            message = completion.choices[0].message
            if not message or not hasattr(message, 'function_call'):
                raise Exception("OpenAI message is missing or does not contain function_call.")

            arguments = message.function_call.arguments
            if arguments is None:
                raise Exception("OpenAI function_call arguments are None.")

            try:
                arguments = json.loads(arguments)
            except Exception as e:
                raise Exception(
                    "OpenAI returned malformed JSON. This is likely due to the completion running out of tokens. " +
