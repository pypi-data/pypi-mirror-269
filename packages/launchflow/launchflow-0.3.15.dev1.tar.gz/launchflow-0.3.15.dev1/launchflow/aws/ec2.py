...
class EC2BaseConnectionInfo(BaseModel):
    """
    Every EC2 resource has some common attributes as well as additional outputs that are resource-specific.
    """
...
class EC2Postgres(EC2[EC2PostgresConnectionInfo]):
    def __init__(self, name: str, *, password: Optional[str] = None) -> None:
        """A Postgres resource running on a VM in AWS EC2.
...
class EC2Redis(EC2[EC2RedisConnectionInfo]):
    """A Redis resource running on a VM in AWS EC2.
    ...
    **Attributes**:
    ...
    redis = lf.aws.EC2Redis("my-redis-instance")
