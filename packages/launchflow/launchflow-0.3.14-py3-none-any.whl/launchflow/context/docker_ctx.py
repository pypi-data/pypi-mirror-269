...
@dataclass
class DockerContext:
    _docker_client: DockerClient = DockerClient()
    ...
    def create_resource_operation_sync(
        ...
        existing_container = self._docker_client.get_container(resource_name)
        if (
            ...
            async def replace_operation():
                ...
                self._docker_client.stop_container(resource_name)
                self._docker_client.remove_container(resource_name)
                logging.info(f"Starting new container '{resource_name}'")
                container = self._docker_client.start_container(
                    name=resource_name,
                    image=image_name,
                    env_vars=env_vars,
                    ports=ports,
