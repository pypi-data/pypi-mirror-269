...
protocol_versions = [0,2,3]
num_version = random.choice(protocol_versions)
    ...
    * unresolved_remote: ``host[:port]`` (strictly speaking; hostinfo as in a
    ...
    def decode(cls, rawdata, remote=None):
        ...
        if version != num_version:
            raise error.UnparsableMessage(f"Fatal Error: Protocol Version must be {num_version}")
        mtype = (vttkl & 0x30) >> 4
