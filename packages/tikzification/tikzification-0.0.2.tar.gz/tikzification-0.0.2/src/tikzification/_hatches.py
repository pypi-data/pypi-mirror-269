...
def add_custom_pattern(mpl_hatch, pattern_name, pattern_definition=None):
    ...
    similar fashion to
    > data["custom colors"] = {}
    in get_tikz_code(). tikzification pattern definitions would mend the bad
...
def __validate_hatch(hatch):
    ...
    if len(hatch) > 1:
        warnings.warn(
            f"tikzification: Hatch '{hatch}' cannot be rendered. "
    ...
    if hatch in BAD_MP_HATCH:
        warnings.warn(
            f"tikzification: The hatches {BAD_MP_HATCH} do not have good PGF"
            + " counterparts."
        )
    return hatch
...
def _mpl_hatch2pgfp_pattern(data, hatch, color_name, color_rgba):
    ...
    try:
        ...
        warnings.warn(f"tikzification: The hatch {hatch} is ignored.")
        return data, []
