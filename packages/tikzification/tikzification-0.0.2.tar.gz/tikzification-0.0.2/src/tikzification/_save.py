...
def get_tikz_code(
    ...
    :param axis_width: If not ``None``, this will be used as figure width within the
                       ...
                       ``tikzification`` will try to preserve the original width/height
    ...
    :param include_disclaimer: Include tikzification disclaimer in the output.
    ...
    and handled:
     ...
     - axes.Axes._tikzification_anchors
    ...
    if include_disclaimer:
        disclaimer = f"This file was created with tikzification v{__version__}."
...
def save_to(filepath: str | Path, *args, encoding: str | None = None, **kwargs):
...
def save(
    ...
    **kwargs,
    ...
    """Similar as `save_to()`, but saves to the output_dir specified in the config.

    :param name: The name of the file to which the TikZ output will be written.
    :type name: str

    :should_clean: Whether to clean the figure before saving.
    :type should_clean: bool

    :param encoding: Sets the text encoding of the output file, e.g. 'utf-8'.
                     For supported values: see ``codecs`` module.
    """
    output_dir = Path(config.get_config("output_dir"))
    tex_folder_name = config.get_config("tex_folder_name", raise_exception=False)

    if tex_folder_name is not None:
        folder_path = output_dir / tex_folder_name
    else:
    ...
    for p in folder_path.glob(name + "*"):
        p.unlink()
    ...
    if should_clean:
        _cleanfigure.clean_figure(fig)

    if (
        ("axis_height" not in kwargs)
        and (axis_width is not None)
        and config.get_config("adjust_to_ratio")
    ):
        ratio = config.get_config("ratio")
        kwargs["axis_height"] = f"{ratio:.2f}*{axis_width}"

    tex_relative_path_to_data = config.get_config(
        "tex_relative_path_to_data", raise_exception=False
    )
    if tex_relative_path_to_data is not None and tex_folder_name is not None:
        tex_relative_path_to_data = f"{tex_relative_path_to_data}/{tex_folder_name}"

    filepath = str(folder_path / f"{name}.tex")
    code = get_tikz_code(
        filepath=str(folder_path / f"{name}.tex"),
        figure=figure,
        tex_relative_path_to_data=tex_relative_path_to_data,
        axis_width=axis_width,
        wrap=config.get_config("wrap"),
        **kwargs,
    )

    code = (
        _tex_comment(
            f"""this code is autogenerated by tikzification v{__version__}!
            Only edit the code if you're sure it will not be overwritten!\n"""
        ) + code
    )
    ...
    if _is_notebook():
        ...
        code = _tex_comment(f"generated from notebook: {path}:\n") + code
...
class _ContentManager:
    """Basic Content Manager for tikzification
...
def _recurse(data, obj):
    ...
    for child in obj.get_children():
        ...
        # See <https://github.com/nschloe/tikzification/issues/277>.
        ...
        if isinstance(child, mpl.axes.Axes):
            ...
            warnings.warn(
                f"tikzification: Don't know how to handle object {type(child)}."
            )
