...
class Parameters:
    ...
    def parse_cmd_arguments(self) -> None:
        ...
        mutually_exclusive_group.add_argument("-gff", dest="gff", type=str, default=None)
        mutually_exclusive_group.add_argument("-gb", dest="gb", type=str, default=None)
        ...
        parser.add_argument("-clust_loc", "--clust_loc", dest="clust_loc", action="store_true")
        parser.add_argument("-reorient_loc", "--reorient_loc", dest="reorient_loc", action="store_true")
        ...
        parser.add_argument("-safl", "--show-all-feature-labels", dest="show-all-feature-labels",
                            action="store_true")  # !
        ...
        if args["linux"]:
            ...
            sys.exit()
        if args["mac"]:
            lovis4u.methods.adjust_paths_for_linux("mac")
            sys.exit()
        if args["help"]:
            ...
            with open(help_message_path, "r") as help_message:
                ...
                sys.exit()
        args_to_keep = ["loci-annotation", "features-annotation"]
    ...
    def define_layout(self, loci) -> None:
        ...
        try:
            ...
            if not annotated_descriptions:
                print("❗Warning message: no description for loci were annotated. Locus label style auto "
                      "changed to 'id'")
